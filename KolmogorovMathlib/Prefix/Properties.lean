...
theorem prefix_cons_cons {a b : α} {p q : List α} (h : a :: p <+: b :: q) : a = b ∧ p <+: q := by
  rcases h with ⟨w, hw⟩
  injection hw with h1 h2
  exact ⟨h1, ⟨w, h2⟩⟩

theorem prefix_nil_false {a : α} {p : List α} (h : a :: p <+: []) : False := by
  rcases h with ⟨w, hw⟩
  contradiction

theorem takeWhile_id_len_eq_or {p q : List Bool} (h : p <+: q) :
    (p.takeWhile id).length = (q.takeWhile id).length ∨
  ...
  | cons a p ih =>
    cases q with
    | nil => exact False.elim (prefix_nil_false h)
    | cons b q =>
      rcases prefix_cons_cons h with ⟨h1, h2⟩
      subst h1
      cases a
      · simp
      · simp only [id_eq, List.takeWhile_cons_of_pos, List.length_cons, Nat.add_right_cancel_iff,
          List.cons.injEq, List.takeWhile_eq_self_iff, Bool.forall_bool, Bool.false_eq_true, imp_false,
          implies_true, and_true, true_and, Order.lt_add_one_iff, Order.add_one_le_iff]
...
theorem take_prefix_take {α} {p q : List α} (n : ℕ) (h : p <+: q) : p.take n <+: q.take n := by
  ...
  | succ n ih =>
    ...
    | cons a p =>
      ...
      | cons b q =>
        rcases prefix_cons_cons h with ⟨h1, h2⟩
        subst h1
        rcases ih h2 with ⟨w, hw⟩
        exact ⟨w, congrArg (List.cons a) hw⟩

theorem drop_prefix_drop {α} {p q : List α} (n : ℕ) (h : p <+: q) : p.drop n <+: q.drop n := by
  induction n generalizing p q with
  | zero => exact h
  | succ n ih =>
    cases p with
...
theorem decodeFirst_mono {p q : BitString} (h : p <+: q) : decodeFirst p <+: decodeFirst q := by
  unfold decodeFirst
  rcases takeWhile_id_len_eq_or h with h_len | ⟨hp_take, h_len_lt⟩
  · rw [h_len]
    apply take_prefix_take
    apply drop_prefix_drop
    exact h
  · have h1 : (p.takeWhile id).length = p.length := congrArg List.length hp_take
    have h2 : (p.takeWhile id).length + 1 > p.length := by omega
    have h3 : p.drop ((p.takeWhile id).length + 1) = [] := List.drop_eq_nil_of_le (by omega)
    simp [h3]

theorem decodeSecond_mono {p q : BitString} (h : p <+: q) : decodeSecond p <+: decodeSecond q := by
  unfold decodeSecond
  rcases takeWhile_id_len_eq_or h with h_len | ⟨hp_take, h_len_lt⟩
  · rw [h_len]
    apply drop_prefix_drop
  ...
  · have h1 : (p.takeWhile id).length = p.length := congrArg List.length hp_take
    ...
    have h3 : p.drop ((p.takeWhile id).length + 1 + (p.takeWhile id).length) = [] := List.drop_eq_nil_of_le (by omega)
    simp [h3]
