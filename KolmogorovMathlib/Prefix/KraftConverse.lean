...
theorem exists_prefixFree_code_of_kraft_le_one (L : List ℕ)
      ...
      := by
        ...
        | succ k ih =>
          ...
          obtain ⟨ σ, hσ₁, hσ₂, hσ₃ ⟩ := ih ( Nat.le_of_succ_le hk ) ( s.erase m )
            ( by rw [ Finset.card_erase_of_mem hm₁, hs_card ]; simp +decide );
          use Fin.cons m σ
          refine ⟨ ?_, ?_, ?_ ⟩
          · intro a
            ...
            · intro b
              ...
              · intro _; rfl
              · intro j hj; exfalso; exact (Finset.mem_erase.mp (hσ₂ j)).1 hj.symm
            · intro i b
              refine Fin.cases ?_ ?_ b
              · intro hi; exfalso; exact (Finset.mem_erase.mp (hσ₂ i)).1 hi
              · intro j hij; exact congrArg Fin.succ (hσ₁ hij)
          · intro a
            refine Fin.cases ?_ ?_ a
            · exact hm₁
            · intro i; exact (Finset.mem_erase.mp (hσ₂ i)).2
          · intro a
            refine Fin.cases ?_ ?_ a
            · intro b
              refine Fin.cases ?_ ?_ b
              · intro h; exfalso; exact lt_irrefl _ h
              · intro j _
                exact hm₂ (σ j) (Finset.mem_erase.mp (hσ₂ j)).2
            · intro i b
              refine Fin.cases ?_ ?_ b
              · intro h; exfalso; exact Nat.not_lt_zero _ h
              · intro j hij
                exact hσ₃ i j (Fin.succ_lt_succ_iff.mp hij)
      exact Exists.elim ( h_perm L.length le_rfl Finset.univ ( by simp +decide ) )
        fun σ hσ ↦ ⟨ σ, hσ.1, hσ.2.2 ⟩;
    exact h_sort;
  -- Define the codeword for each index using the sorted sequence.
  obtain ⟨f, hf⟩ : ∃ f : Fin L.length → BitString, (∀ i, (f i).length = L.get (σ i)) ∧
    (∀ i j, i < j → ¬ IsStrictPrefix (f i) (f j)) ∧ (∀ i j, i ≠ j → f i ≠ f j)
  := by
    -- Define the codeword for each index using the sorted sequence and the Kraft inequality.
    obtain ⟨a, ha⟩ : ∃ a : Fin L.length → ℕ, (∀ i, a i < 2 ^ (L.get (σ i))) ∧
      (∀ i j, i < j → a j ≥ (a i + 1) * 2 ^ (L.get (σ j) - L.get (σ i)))
    := by
      refine ⟨ fun i ↦ ∑ j ∈ Finset.univ.filter ( fun j ↦ j < i ),
        2 ^ ( L.get ( σ i ) - L.get ( σ j ) ), ?_, ?_ ⟩ <;> simp +decide only [ Finset.sum_filter ]
      · intro i
        have h_sum_lt : ∑ a ∈ Finset.univ.filter (fun a ↦ a < i),
          (2 : ℝ) ^ (L.get (σ i) - L.get (σ a)) < 2 ^ L.get (σ i)
        := by
          have h_sum_lt : ∑ a ∈ Finset.univ.filter (fun a ↦ a < i),
            (2 : ℝ) ^ (L.get (σ i) - L.get (σ a)) ≤ 2 ^ L.get (σ i) *
            (∑ a ∈ Finset.univ.filter (fun a ↦ a < i), (1 / 2 : ℝ) ^ L.get (σ a))
          := by
            rw [ Finset.mul_sum _ _ _ ]; refine Finset.sum_le_sum fun j hj ↦ ?_
            rw [ ← Nat.sub_add_cancel ( show L.get ( σ j ) ≤ L.get ( σ i ) from ?_ ) ]
            · norm_num [ pow_add, pow_mul ]; ring_nf
              norm_num [ mul_assoc, mul_comm, mul_left_comm, ← mul_pow ]
              norm_num [ ← mul_assoc, ← mul_pow ]
            · aesop
          have h_sum_lt : ∑ a ∈ Finset.univ.filter (fun a ↦ a < i), (1 / 2 : ℝ) ^ L.get (σ a) <
            ∑ a ∈ Finset.univ, (1 / 2 : ℝ) ^ L.get (σ a)
          := by
            rw [ ← Finset.sum_sdiff ( Finset.filter_subset ( fun a ↦ a < i ) Finset.univ ) ];
            exact lt_add_of_pos_left _
              ( Finset.sum_pos ( fun x hx ↦ by positivity ) ⟨ i, by aesop ⟩ );
          have h_sum_lt : ∑ a ∈ Finset.univ, (1 / 2 : ℝ) ^ L.get (σ a) ≤ 1 := by
            convert hK using 1;
            have h_sum_lt : ∑ a ∈ Finset.univ, (1 / 2 : ℝ) ^ L.get (σ a) =
              ∑ a ∈ Finset.image σ Finset.univ, (1 / 2 : ℝ) ^ L.get a
            := by
              rw [ Finset.sum_image <| by tauto ];
            rw [ h_sum_lt, Finset.eq_of_subset_of_card_le ( Finset.image_subset_iff.mpr fun i _ ↦
              Finset.mem_univ ( σ i ) ) ( by rw [ Finset.card_image_of_injective _ hσ.1 ] ) ];
            norm_num [ List.sum_map_mul_right ];
          nlinarith [ pow_pos ( zero_lt_two' ℝ ) ( L.get ( σ i ) ) ];
        norm_cast at *; simp_all +decide only [ Finset.sum_ite, Finset.sum_const_zero, add_zero ]
      · intro i j hij; rw [ add_mul, one_mul ]; simp +decide only [ Finset.sum_ite,
        Finset.sum_const_zero, add_zero ]
        rw [ show ( Finset.filter ( fun x ↦ x < j ) Finset.univ : Finset ( Fin L.length ) ) =
          Finset.filter ( fun x ↦ x < i ) Finset.univ ∪ { i } ∪
          Finset.filter ( fun x ↦ i < x ∧ x < j ) Finset.univ from ?_,
          Finset.sum_union, Finset.sum_union ] <;>
        norm_num [ Finset.sum_singleton, Finset.sum_union, Finset.sum_filter,
          Finset.sum_range_succ, Nat.pow_succ', mul_assoc, mul_comm, mul_left_comm,
        ...
        · exact Finset.disjoint_left.mpr fun x hx₁ hx₂ ↦ lt_asymm (
          Finset.mem_filter.mp hx₁ |>.2 ) ( Finset.mem_filter.mp hx₂ |>.2.1 );
        · grind;
    refine ⟨ fun i ↦ natToCode ( L.get ( σ i ) ) ( a i ), ?_, ?_, ?_ ⟩
    · simp_all +decide only [one_div, inv_pow, List.get_eq_getElem]
      grind +locals;
    · simp_all +decide only [one_div, inv_pow, List.get_eq_getElem, IsStrictPrefix, ne_eq, not_and,
      Decidable.not_not]
      intro i j hij h;
      -- By the properties of `natToCode`, if `natToCode L[σ i] (a i)` is a prefix
      -- of `natToCode L[σ j] (a j)`, then `a j / 2^(L[σ j] - L[σ i]) = a i`.
      have h_div : a j / 2 ^ (L[σ j] - L[σ i]) = a i := by
        have h_div : ∀ (L1 L2 : ℕ) (a1 a2 : ℕ), L1 ≤ L2 → a1 < 2 ^ L1 → a2 < 2 ^ L2 →
          natToCode L1 a1 <+: natToCode L2 a2 → a2 / 2 ^ (L2 - L1) = a1
        := by
      ...
      have := ha.2 i j hij;
      exact absurd h_div
        ( Nat.ne_of_gt <| Nat.le_div_iff_mul_le ( by positivity ) |>.2 <| by linarith! );
    · simp_all +decide only [one_div, inv_pow, List.get_eq_getElem, ne_eq]
      intro i j hij; contrapose! hij; have := ha.2 i j; have
        := ha.2 j i; simp_all +decide only [implies_true];
      ...
      have h_eq : a i = a j := by
        ...
        := by
          ...
          have h_eq : ∀ (L : ℕ) (a b : ℕ), a < 2 ^ L → b < 2 ^ L → (List.map (fun i ↦
            a.testBit i) (List.range L).reverse) =
              (List.map (fun i ↦ b.testBit i) (List.range L).reverse) → a = b := by
            intros L a b ha hb h_eq; exact (by
            refine Nat.eq_of_testBit_eq fun i ↦ ?_;
            by_cases hi : i < L
            · simp_all +decide only [List.get_eq_getElem, List.map_reverse, List.reverse_inj,
              List.map_inj_left, List.mem_range]
            · simp_all +decide only [List.get_eq_getElem, List.map_reverse, List.reverse_inj,
              List.map_inj_left, List.mem_range, not_lt]
              rw [ Nat.testBit_eq_false_of_lt, Nat.testBit_eq_false_of_lt ] <;> linarith [
                Nat.pow_le_pow_right two_pos hi ]);
          exact h_eq L a b ha hb ‹_›);
        grind;
      exact le_antisymm ( le_of_not_gt fun hi ↦ by have := ha.2 _ _ hi; aesop )
        ( le_of_not_gt fun hj ↦ by have := ha.2 _ _ hj; aesop );
  ...
  · rintro _ ⟨i, rfl⟩ _ ⟨j, rfl⟩ hij
    ...
    · have h_eq : i = j := by
        ...
        exact h2
      rw [h_eq]
    · have h_len_eq : (f (σ_equiv.symm i)).length ≤ (f (σ_equiv.symm j)).length
      := List.IsPrefix.length_le hij
