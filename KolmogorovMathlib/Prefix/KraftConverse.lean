...
theorem exists_prefixFree_code_of_kraft_le_one (L : List ℕ)
  ...
  obtain ⟨f, hf⟩ : ∃ f : Fin L.length → BitString, (∀ i, (f i).length = L.get (σ i)) ∧
    ...
    obtain ⟨a, ha⟩ : ∃ a : Fin L.length → ℕ, (∀ i, a i < 2 ^ (L.get (σ i))) ∧
        (∀ i j, i < j → a j ≥ (a i + 1) * 2 ^ (L.get (σ j) - L.get (σ i))) := by
      refine ⟨ fun i => ∑ j ∈ Finset.univ.filter ( fun j =>
          j < i ), 2 ^ ( L.get ( σ i ) - L.get (
              σ j ) ), ?_, ?_ ⟩ <;> simp_all only [one_div, inv_pow, List.get_eq_getElem,
                  Finset.sum_filter, ge_iff_le];
      · intro i
        have h_sum_lt : ∑ a ∈ Finset.univ.filter (fun a =>
            a < i), (2 : ℝ) ^ (L.get (σ i) - L.get (σ a)) < 2 ^ L.get (σ i) := by
          have h_sum_lt : ∑ a ∈ Finset.univ.filter (fun a =>
              a < i), (2 : ℝ) ^ (L.get (σ i) - L.get (σ a)) ≤ 2 ^ L.get (σ i) * (∑ a ∈
                  Finset.univ.filter (fun a => a < i), (1 / 2 : ℝ) ^ L.get (σ a)) := by
            ...
            · aesop;
          have h_sum_lt : ∑ a ∈ Finset.univ.filter (fun a =>
              a < i), (1 / 2 : ℝ) ^ L.get (σ a) < ∑ a ∈ Finset.univ,
                  (1 / 2 : ℝ) ^ L.get (σ a) := by
            rw [ ← Finset.sum_sdiff ( Finset.filter_subset ( fun a => a < i ) Finset.univ ) ];
            exact lt_add_of_pos_left _ ( Finset.sum_pos ( fun x hx =>
                by positivity ) ⟨ i, by aesop ⟩ );
          have h_sum_lt : ∑ a ∈ Finset.univ, (1 / 2 : ℝ) ^ L.get (σ a) ≤ 1 := by
            convert hK using 1;
            have h_sum_lt : ∑ a ∈ Finset.univ,
                (1 / 2 : ℝ) ^ L.get (σ a) = ∑ a ∈ Finset.image σ Finset.univ,
                    (1 / 2 : ℝ) ^ L.get a := by
              rw [ Finset.sum_image <| by tauto ];
            rw [ h_sum_lt, Finset.eq_of_subset_of_card_le (
                Finset.image_subset_iff.mpr fun i _ =>
                    Finset.mem_univ ( σ i ) ) (
                        by rw [ Finset.card_image_of_injective _ hσ.1 ] ) ] ;
                            norm_num [ List.sum_map_mul_right ];
            norm_num [ ← inv_pow ];
          nlinarith [ pow_pos ( zero_lt_two' ℝ ) ( L.get ( σ i ) ) ];
        norm_cast at * ; simp_all +decide [ Finset.sum_ite ];
      · intro i j hij; rw [ add_mul, one_mul ] ;
          simp only [Finset.sum_ite, not_lt, Finset.sum_const_zero, add_zero] at * ;
        rw [ show ( Finset.filter ( fun x =>
            x < j ) Finset.univ : Finset ( Fin L.length ) ) = Finset.filter ( fun x =>
                x < i ) Finset.univ ∪ { i } ∪ Finset.filter ( fun x => i < x ∧
                    x < j ) Finset.univ from ?_, Finset.sum_union, Finset.sum_union ] <;>
                        norm_num [ Finset.sum_singleton, Finset.sum_union, Finset.sum_filter,
                            Finset.sum_range_succ, Nat.pow_succ', mul_assoc, mul_comm,
        ...
        · grind;
    refine ⟨ fun i => natToCode ( L.get ( σ i ) ) (
        a i ), ?_, ?_, ?_ ⟩ <;> simp_all only [one_div, inv_pow, List.get_eq_getElem,
            ge_iff_le, IsStrictPrefix, ne_eq, not_and];
    · grind +locals;
    · intro i j hij h;
    ...
    · intro i j hij; contrapose! hij; have := ha.2 i j; have := ha.2 j i;
  ...
  · rintro _ ⟨i, rfl⟩ _ ⟨j, rfl⟩ hij
    ...
    · have h_neq : f (σ_equiv.symm i) ≠ f (σ_equiv.symm j) := fun heq =>
        hf.2.2 _ _ (ne_of_lt h) heq
