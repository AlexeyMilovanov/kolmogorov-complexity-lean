...
In standard literature, proofs in AIT often rely on informal appeals to the Church-Turing thesis and asymptotic bounds (big-O notation). This repository bridges the gap between abstract computability theory and information theory by explicitly constructing universal Turing machines, tracking additive constants, and rigorously proving the uncomputability of information content.

Built on top of Mathlib's computability theory (`Mathlib.Computability.Partrec`), this project formalizes the definitions of both **plain** ($K(x)$) and **conditional** ($K(x|y)$) Kolmogorov complexity, treating Turing machines as partial recursive functions.

## Key Formalizations
The project formalizes the following core components of Algorithmic Information Theory:

1. **The Universal Machine and The Invariance Theorem (Kolmogorov-Solomonoff-Chaitin):**
   Construction of a Conditional Universal Machine using a unary prefix-coding scheme. The project includes a formal proof of its optimality: the capability to simulate any other computable decompressor with a strictly bounded additive penalty to the program length. This establishes the invariance of Kolmogorov complexity up to an additive constant.
...
4. **The Uncomputability Theorem (Berry's Paradox):**
   The primary result of the project. A formal proof that the plain Kolmogorov complexity function is Turing-uncomputable. The proof models Berry's Paradox by creating a computable search algorithm that looks for complex strings, deriving a mathematical contradiction between the logarithmic length of the search parameter and the exponential complexity of the generated string.
...
* **Monadic Computability (`Part`):** Turing machines are modeled using Lean's `Part` monad (partial functions), allowing for standard handling of halting/non-halting computations and integration with Mathlib's `Partrec` API.
...
* **Computability Proofs:** The file provides exact, combinator-based proofs (`Primrec.prefixLen`, `Primrec₂.listDrop`) ensuring these functions can be executed by our theoretical Turing machines.
...
The `Core/` directory represents the semantic heart of the project. It defines the central concepts of Algorithmic Information Theory (AIT), constructs the Universal Turing Machine, and culminates in the formal proof of the Invariance Theorem.
...
This file translates the abstract mathematical concepts of AIT into rigorous Lean 4 types. By defining Turing machines as partial recursive functions and utilizing the Extended Naturals (`ENat`), it creates a framework where non-terminating programs and uncomputable values are handled elegantly without logical paradoxes.

**Key Components:**
* `Machine`: Defined as `BitString × BitString →. BitString`. The use of the `Part` monad (`→.`) perfectly encapsulates the fact that Turing machines might not halt.
* `produces`: Formalizes the relationship "machine $D$ outputs $x$ given program $p$ and context $y$" using the monadic membership operator (`x ∈ D (p, y)`).
...
* `isOptimalConditional`: Defines the concept of universality. A machine is optimal if it is computable (`isDecompressor`) and can simulate any other computable machine with at most a constant additive overhead to the program length.

---

### 5. `Core/UniversalMachine.lean`
**Purpose:** Constructing the Conditional Universal Turing Machine.



To prove that an optimal machine exists, one must be explicitly constructed. This file builds a Universal Machine $U$ that multiplexes an infinite number of Turing machines onto a single tape using a prefix-free encoding. It bridges the gap between a human-readable definition and a strict mathematical combinator required for computability proofs.

**Key Components:**
* `unaryPrefix`: Implements unary prefix coding ($n$ is encoded as $n$ `true`s followed by a `false`). This allows the universal machine to read the index of the target machine, know exactly where the index ends, and treat the rest of the tape as the program.
* `universalMachine` & `universalMachineCombinator`: Defines the machine in two ways. The first is readable and utilizes `match` statements; the second uses strict monadic binds (`Part.bind`). The `universalMachine_eq_combinator` lemma proves they are identical.
* `isDecompressor_universalMachine`: The heavy-lifting proof of the file. By meticulously composing `Partrec` (partial recursive) combinators, it proves that the Universal Machine is itself fully Turing-computable.
...
This is the foundational theorem of Algorithmic Information Theory. It proves that the choice of programming language or Turing machine does not matter up to an additive constant. Because we have an optimal universal machine, complexity is an objective mathematical property of the string itself.

**Key Components:**
* `exists_code_of_isDecompressor`: Connects our abstract definition of computable machines with Mathlib's Gödel numbering (`Nat.Partrec.Code`). It proves that every computable machine has a numerical code whose evaluation perfectly mirrors the machine's behavior.
...
* `exists_isOptimalConditional`: The main theorem. It proves that the `universalMachine` satisfies the `isOptimalConditional` predicate. It explicitly constructs the proof by showing that to simulate any machine $D$, $U$ simply prepends the unary prefix of $D$'s Gödel number to the program. The length overhead $c$ is exactly the length of this prefix, which is a constant independent of the string being compressed.

## File-by-File Documentation: Complexity

The `Complexity/` directory contains the high-level theorems and applications of Algorithmic Information Theory. Having established the foundational ontology and the optimal Universal Turing Machine in `Core/`, these files formalize the mathematical behavior of information, culminating in the proof of uncomputability.

### 7. `Complexity/Properties.lean`
**Purpose:** Establishing the fundamental information inequalities.

This file proves the basic laws of algorithmic information processing. It shows how complexity behaves under composition, self-reference, and the application of computable functions. By relying on the optimality of the Universal Machine $U$, the proofs abstract away the underlying Turing machine mechanics into clean, algebraic bounds.
...
* `compressibleWords` & `card_compressibleWords_lt`: Proves that a Turing machine can generate strictly fewer than $2^{k+1}$ strings of complexity $\le k$.
...
While Turing machines natively operate on bit strings, mathematical paradoxes (like Berry's Paradox) are traditionally formulated using Natural Numbers. This file seamlessly lifts the theory to $\mathbb{N}$ using the dyadic bijection formalized in the `Foundation/` directory.
...
This is the culminating file of the project. It formally proves that no Turing machine can compute the plain Kolmogorov complexity function. The proof rigorously models a computational version of Berry's Paradox: *"Find the smallest number whose complexity is greater than $2^k$"*. 
