...
noncomputable def pairMarginal (M : Map) (x z : BitString) : ℝ≥0∞ :=
  ∑' y : BitString, aprioriMeasure M (pairCode x y) z

/-- Unfolding lemma for `pairMarginal`. -/
theorem pairMarginal_def (M : Map) (x z : BitString) :
    pairMarginal M x z = ∑' y : BitString, aprioriMeasure M (pairCode x y) z :=
  rfl
...
theorem tsum_pairMarginal_le_one (M : Map) (z : BitString) (hM : IsPrefixMachine M) :
    (∑' x : BitString, pairMarginal M x z) ≤ 1 := by
  calc
...
`tsum` linearity. -/
theorem pairSection_tsum (M : Map) (x z : BitString) (k : ℕ) :
    ∑' y : BitString, (aprioriMeasure M (pairCode x y) z * (2 : ℝ≥0∞) ^ k)
      = pairMarginal M x z * (2 : ℝ≥0∞) ^ k := by
...
theorem pairSection_tsum_le_of_marginal_scaled (M : Map) (x z : BitString) (k d : ℕ)
    (h : pairMarginal M x z * (2 : ℝ≥0∞) ^ k ≤ (2 : ℝ≥0∞) ^ d) :
    ∑' y : BitString, (aprioriMeasure M (pairCode x y) z * (2 : ℝ≥0∞) ^ k)
      ≤ (2 : ℝ≥0∞) ^ d := by
  rw [pairSection_tsum]
  exact h
...
theorem pairSection_tsum_le_one_of_marginal (M : Map) (x : BitString) (k : ℕ)
  ...
  rw [pairSection_tsum]
  calc pairMarginal M x z * (2 : ℝ≥0∞) ^ k
