...
lemma computable_finset_range_sup {α : Type*} [Primcodable α]
  ...
  have h_eq : ∀ (n : ℕ) (f : ℕ → ℕ), (Finset.range n).sup f = (List.range n).foldr (fun s' acc ↦ max (f s') acc) 0 := by
    intro n f
    induction n with
    | zero => rfl
    | succ n ih =>
      simp only [Finset.range_add_one, Finset.sup_insert, List.range_succ, List.foldr_append, List.foldr_cons, List.foldr_nil, max_eq_left (Nat.zero_le _)]
      rw [ih]
  ...
  have h_step : Computable₂ (fun (p : α × ℕ) (q : ℕ × ℕ) ↦ max (f p.1 q.1) q.2) := by
    have h1 : Computable (fun a : (α × ℕ) × (ℕ × ℕ) ↦ f a.1.1 a.2.1) :=
      hf.comp (Computable.pair (Computable.fst.comp Computable.fst) (Computable.fst.comp Computable.snd))
  ...
  have h_fold : Computable (fun p : α × ℕ ↦ (List.range p.2).foldr (fun s' acc ↦ max (f p.1 s') acc) 0) := by
    exact h_rec.of_eq (fun p ↦ by
      induction p.2 with
      | zero => rfl
      | succ n ih =>
        simp only [List.range_succ, List.foldr_append, List.foldr_cons, List.foldr_nil, max_eq_left (Nat.zero_le _)]
        rw [ih]
        have h_comm : ∀ x y z : ℕ, max x (max y z) = max y (max x z) := by
          intro x y z; rw [← Nat.max_assoc, Nat.max_comm x y, Nat.max_assoc]
        generalize List.range n = L
...
lemma approxEnum_computable :
  ...
  rotate_right
  exact fun p ↦
    (Finset.range (p.2.1 + 1)).sup fun s' ↦
    ...
    ∃ i, ∀ out ctx,
      ...
      (⨆ s, dyadicValue (approx s out ctx) s) := by
        ...
        obtain ⟨c, hc⟩ : ∃ c : Nat.Partrec.Code, ∀ s out ctx, Nat.Partrec.Code.eval c (Encodable.encode (s, out, ctx)) = some (approx s out ctx) := by
          ...
          obtain ⟨ c, hc ⟩ := this.mp (Partrec.nat_iff.mp hh₁.partrec)
          exact ⟨ c, fun s out ctx ↦ by simp +decide [ ← hh₂, hc ] ⟩;
        refine ⟨ Encodable.encode c, fun out ctx ↦ le_antisymm ?_ ?_ ⟩ <;>
          simp_all +decide only [Encodable.encode_prod_val, Encodable.encode_nat, Part.coe_some, approxEnum,
            Denumerable.decode_eq_ofNat, Denumerable.ofNat_encode, Option.getD_some, iSup_le_iff];
        · intro i
          have h_term : ∀ s' ∈ Finset.range (i + 1), dyadicValue (match Nat.Partrec.Code.evaln i c (Nat.pair s' (Nat.pair (Encodable.encode out) (Encodable.encode ctx))) with
            ...
            | none => 0) i ≤ ⨆ s, dyadicValue (approx s out ctx) s := by
              ...
              have h_term : Nat.Partrec.Code.evaln i c (Nat.pair s' (Nat.pair (Encodable.encode out) (Encodable.encode ctx))) = some (approx s' out ctx) ∨ Nat.Partrec.Code.evaln i c (Nat.pair s' (Nat.pair (Encodable.encode out) (Encodable.encode ctx))) = none := by
                cases heval : Nat.Partrec.Code.evaln i c ( Nat.pair s' ( Nat.pair ( Encodable.encode out ) ( Encodable.encode ctx ) ) )
                · exact Or.inr rfl
                · apply Or.inl
              ...
              cases h_term <;>
                simp +decide only [dyadicValue, Nat.cast_mul, Nat.cast_pow, Nat.cast_ofNat, ge_iff_le,
                  Nat.cast_zero, ENNReal.zero_div, zero_le, *];
          ...
          have h_sup : ∀ {S : Finset ℕ} {f : ℕ → ℕ}, (∀ s' ∈ S, dyadicValue (f s') i ≤ ⨆ s, dyadicValue (approx s out ctx) s) → dyadicValue (S.sup f) i ≤ ⨆ s, dyadicValue (approx s out ctx) s := by
            ...
            | @insert s' S' hs' ih =>
              ...
              cases max_cases ( f s' ) ( S'.sup f ) <;>
                simp +decide only [Finset.sup_insert, sup_of_le_left, Finset.mem_insert, or_false, *]
        ...
        · intro s;
          ...
          · unfold dyadicValue; norm_num [ pow_add, pow_one, pow_mul, mul_assoc, mul_comm, mul_left_comm, div_eq_mul_inv ];
            ...
            simp +decide only [ne_eq, pow_eq_zero_iff', OfNat.ofNat_ne_zero, false_and, not_false_eq_true,
              ENNReal.pow_eq_top_iff, or_self, ENNReal.mul_inv, mul_comm, mul_left_comm];
...
lemma dyadicValue_mixtureDyadicApprox
  ...
  · simp +decide only [mul_assoc, mul_comm, ne_eq, OfNat.ofNat_ne_zero, not_false_eq_true, true_or, mul_eq_zero,
      pow_eq_zero_iff', false_and, Nat.div_eq_zero_iff, Order.lt_two_iff, false_or, not_le, or_self, ENNReal.mul_inv,
      ENNReal.pow_eq_top_iff, ENNReal.inv_pow, Nat.cast_mul, Nat.cast_pow, Nat.cast_ofNat, mul_left_comm, div_eq_mul_inv];
