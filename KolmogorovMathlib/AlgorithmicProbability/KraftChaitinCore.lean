...
lemma dyadicValue_cumNum (S n : ℕ) (ctx : BitString) (h : n ≤ S) :
  ...
  exact Finset.sum_congr rfl fun i hi => by rw [ dyadicValue_evNum_scale i S ctx ( Nat.le_trans ( Nat.unpair_right_le i ) ( by linarith [ Finset.mem_range.mp hi ] ) ) ] ;
...
lemma evOut_computable : Computable evOut := by
  ...
  have h_computable : Primrec₂ (fun (a : ℕ) (b : BitString) => if Encodable.encode b = a then some b else none) := by
...
lemma incNum_computable (hcomp : Computable
  ...
  · convert hcomp.comp ( Computable.const 0 |> Computable.pair <| Computable.fst.comp Computable.snd |> Computable.pair <| Computable.snd.comp Computable.snd ) using 1;
  · have h_comp : Computable (fun p : ℕ × BitString × BitString => approx (p.1 + 1) p.2.1 p.2.2) := by
      convert hcomp.comp ( Computable.pair ( Computable.succ.comp Computable.fst ) ( Computable.snd ) ) using 1;
    have h_comp : Computable (fun p : ℕ × BitString × BitString => 2 * approx p.1 p.2.1 p.2.2) := by
      have h_comp : Computable (fun p : ℕ => 2 * p) := by
        have h_comp : Primrec (fun p : ℕ × ℕ => p.1 + p.2) := by
          exact Primrec.nat_add.comp ( Primrec.fst ) ( Primrec.snd );
        grind +suggestions;
      exact h_comp.comp hcomp;
    have h_comp : Computable (fun p : (BitString × BitString) × ℕ => approx (p.2 + 1) p.1.1 p.1.2 - 2 * approx p.2 p.1.1 p.1.2) := by
      ...
      convert h_comp.comp ( Computable.pair ( ‹Computable fun p : ℕ × BitString × BitString => approx ( p.1 + 1 ) p.2.1 p.2.2›.comp ( Computable.snd.pair ( Computable.fst.comp ( Computable.fst ) |> Computable.pair <| Computable.snd.comp ( Computable.fst ) ) ) ) ( ‹Computable fun p : ℕ × BitString × BitString => 2 * approx p.1 p.2.1 p.2.2›.comp ( Computable.snd.pair ( Computable.fst.comp ( Computable.fst ) |> Computable.pair <| Computable.snd.comp ( Computable.fst ) ) ) ) ) using 1;
    convert h_comp.comp ( Computable.pair ( Computable.snd.comp Computable.fst ) ( Computable.snd ) ) using 1;
...
lemma evNum_computable (hcomp : Computable
  ...
  · have := incNum_computable hcomp;
    convert this.comp ( show Computable fun p : ( ℕ × BitString ) × BitString => ( evK p.1.1, p.2, p.1.2 ) from ?_ ) using 1;
    exact Computable.pair ( evK_computable.comp ( Computable.fst.comp Computable.fst ) ) ( Computable.pair ( Computable.snd ) ( Computable.snd.comp Computable.fst ) )
...
lemma truncGTerm_computable (hcomp : Computable
  ...
  · convert h.comp ( Computable.pair ( Computable.fst.comp Computable.fst ) ( Computable.pair ( Computable.snd ) ( Computable.snd.comp Computable.fst ) ) ) using 1;
    grind
  ...
  · convert incNum_computable_uniform b hb using 1;
    ...
    · convert h.comp _ using 1;
      ...
      · apply Computable.pair;
        · exact Computable.fst;
        · apply Computable.pair;
          · exact Computable.snd.comp ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd ) ) );
          · apply Computable.pair;
      ...
      have h_exp_computable : Computable (fun q : (ℕ × ℕ × BitString × BitString) × ℕ => 2 ^ (d + q.1.2.1)) := by
        ...
        have h_add_computable : Computable (fun q : ℕ => d + q) := by
          ...
          | succ d ih =>
            ...
            exact funext fun n => by simp +decide [ add_comm, add_left_comm ] ;
...
lemma construct_out_partrec (req : BitString → ℕ → Option (BitString × ℕ))
  ...
  have h_partrec : Computable (fun p : (BitString × BitString) × ℕ => Option.map Prod.fst (req p.1.2 p.2)) := by
    convert h_if_computable.comp ( hreqcomp.comp ( Computable.snd.comp ( Computable.fst ) |> Computable.pair <| Computable.snd ) ) using 1;
...
lemma geomCross_succ_of_geomCross {approx : ℕ → BitString → BitString → ℕ}
  ...
  · convert hmono s o ctx |> le_trans _ using 1;
    rotate_left;
    exact 1 / 2 ^ ‹_›;
    · convert ENNReal.div_le_div ( show ( 2 ^ s : ENNReal ) ≤ approx s o ctx * 2 ^ ‹_› from ?_ ) le_rfl using 1;
  ...
  obtain ⟨l, hl⟩ : ∃ l : ℕ, 1 ≤ l ∧ ∃ s : ℕ, geomCross approx out ctx l s ∧ ∀ l' : ℕ, l' < l → ¬∃ s' : ℕ, geomCross approx out ctx l' s' := by
    have hL : ∃ l : ℕ, 1 ≤ l ∧ ∃ s : ℕ, geomCross approx out ctx l s := by
      ...
      obtain ⟨s, hs⟩ : ∃ s : ℕ, dyadicValue (approx s out ctx) s > 0 := by
        ...
        exact not_forall_not.mp fun h => h_pos.ne' <| hsup out ctx ▸ by simp +decide [ show ∀ s : ℕ, dyadicValue ( approx s out ctx ) s = 0 from fun s => le_antisymm ( le_of_not_gt fun hs => h s hs ) ( zero_le _ ) ] ;
