...
lemma dyadicValue_cumNum (S n : ℕ) (ctx : BitString) (h : n ≤ S) :
  ...
  exact Finset.sum_congr rfl fun i hi ↦
    dyadicValue_evNum_scale i S ctx ( Nat.le_trans ( Nat.unpair_right_le i )
...
lemma dyadicValue_truncGapprox (d S : ℕ) (out ctx : BitString) :
  ...
  · intro b hb₁ hb₂ hb₃; contrapose! hb₂; simp_all +decide [ truncCum ];
    ...
    exact le_of_eq ( dyadicValue_cumNum S ( b + 1 ) ctx ( by linarith ) );
  · intro a ha₁ ha₂ ha₃; rw [ ← dyadicValue_evNum_scale a S ctx ( Nat.le_of_lt ( Nat.unpair_right_le a |> lt_of_le_of_lt <| ha₁ ) ) ]; ring_nf;
...
`evOut` is computable.
-/
lemma evOut_computable : Computable evOut :=
  (Primrec.decode₂ (α := BitString)).to_comp.comp
    (Computable.fst.comp Computable.unpair)

/-
`evK` is computable.
-/
lemma evK_computable : Computable evK :=
...
lemma incNum_computable (hcomp : Computable
  ...
  have hg : Computable (fun p : ℕ × BitString × BitString ↦ approx 0 p.2.1 p.2.2) :=
    hcomp.comp ( Computable.const 0 |> Computable.pair <| Computable.fst.comp Computable.snd |> Computable.pair <| Computable.snd.comp Computable.snd )
  have h_succ : Computable (fun p : ℕ × BitString × BitString ↦ approx (p.1 + 1) p.2.1 p.2.2) :=
    hcomp.comp ( Computable.pair ( Computable.succ.comp Computable.fst ) ( Computable.snd ) )
  have h_dbl : Computable (fun p : ℕ × BitString × BitString ↦ 2 * approx p.1 p.2.1 p.2.2) := by
    have h_two_mul : Computable (fun p : ℕ ↦ 2 * p) :=
      (Primrec.nat_mul.comp (Primrec.const 2) Primrec.id).to_comp
    exact h_two_mul.comp hcomp
  have h_sub : Computable₂ (fun a b : ℕ ↦ a - b) :=
    (Primrec.nat_sub.comp Primrec.fst Primrec.snd).to_comp
  have hreshape : Computable (fun p : (BitString × BitString) × ℕ ↦ (p.2, p.1.1, p.1.2)) :=
    Computable.snd.pair ( Computable.fst.comp ( Computable.fst ) |> Computable.pair <| Computable.snd.comp ( Computable.fst ) )
  have hA := h_succ.comp hreshape
  have hB := h_dbl.comp hreshape
  have h_body := Computable₂.comp h_sub hA hB
  have hreshape2 : Computable (fun p : (ℕ × BitString × BitString) × ℕ ↦ (p.1.2, p.2)) :=
    Computable.pair ( Computable.snd.comp Computable.fst ) ( Computable.snd )
  have hh : Computable₂ (fun (p : ℕ × BitString × BitString) (k : ℕ) ↦
      approx (k + 1) p.2.1 p.2.2 - 2 * approx k p.2.1 p.2.2) :=
    (h_body.comp hreshape2).of_eq (fun _ ↦ rfl)
  exact (Computable.nat_casesOn Computable.fst hg hh).of_eq fun p ↦ by cases p.1 <;> rfl
...
lemma evNum_computable (hcomp : Computable
  ...
  convert Computable.option_casesOn ( show Computable fun p : ℕ × BitString ↦ evOut p.1 from ?_ ) ( show Computable fun p : ℕ × BitString ↦ 0 from ?_ ) ( show Computable₂ fun p : ℕ × BitString ↦ fun out : BitString ↦ incNum approx ( evK p.1 ) out p.2 from ?_ ) using 1;
  · exact funext fun p ↦ by unfold evNum; aesop;
  · exact evOut_computable.comp ( Computable.fst );
  · exact Computable.const 0;
  · have hinc := incNum_computable hcomp
    have hmap : Computable (fun x : (ℕ × BitString) × BitString ↦ (evK x.1.1, x.2, x.1.2)) :=
      Computable.pair ( evK_computable.comp ( Computable.fst.comp Computable.fst ) )
        ( Computable.pair ( Computable.snd ) ( Computable.snd.comp Computable.fst ) )
    exact (hinc.comp hmap).of_eq (fun _ ↦ rfl)
...
lemma cumNum_computable (hcomp : Computable
  ...
  have h_computable : Computable (fun p : ℕ × ℕ × BitString ↦ ∑ i ∈ Finset.range p.2.1, evNum approx i p.2.2 * 2 ^ (p.1 - evK i)) := by
    have h_g : Computable (fun p : (ℕ × ℕ × BitString) × ℕ ↦ evNum approx p.2 p.1.2.2 * 2 ^ (p.1.1 - evK p.2)) := by
      have h_g : Computable (fun p : (ℕ × ℕ × BitString) × ℕ ↦ evNum approx p.2 p.1.2.2) := by
        convert evNum_computable hcomp |> Computable.comp <| _ using 1;
        rotate_left;
        exact fun p ↦ ( p.2, p.1.2.2 );
        · exact Computable.pair ( Computable.snd ) ( Computable.snd.comp ( Computable.snd.comp ( Computable.fst ) ) );
        · rfl;
      have h_g : Computable (fun p : (ℕ × ℕ × BitString) × ℕ ↦ 2 ^ (p.1.1 - evK p.2)) := by
        have h_g : Computable (fun p : ℕ × ℕ ↦ 2 ^ (p.1 - evK p.2)) := by
          convert Primrec.to_comp ( show Primrec ( fun p : ℕ × ℕ ↦ 2 ^ ( p.1 - evK p.2 ) ) from ?_ ) using 1;
          convert Primrec.comp ( show Primrec ( fun n ↦ 2 ^ n ) from ?_ ) ( show Primrec ( fun p : ℕ × ℕ ↦ p.1 - evK p.2 ) from ?_ ) using 1;
        ...
        convert h_g.comp ( Computable.fst.comp ( Computable.fst ) |> Computable.pair <| Computable.snd ) using 1;
      -- The product of two computable functions is computable.
      have h_prod : ∀ (f g : (ℕ × ℕ × BitString) × ℕ → ℕ), Computable f → Computable g → Computable (fun p ↦ f p * g p) := by
        intros f g hf hg
      ...
      grind
    have hg2 : Computable₂ (fun (p : ℕ × ℕ × BitString) (i : ℕ) ↦
        evNum approx i p.2.2 * 2 ^ (p.1 - evK i)) := h_g.of_eq (fun _ ↦ rfl)
    exact computable_range_sum _ hg2 (fun p ↦ p.2.1) (Computable.fst.comp Computable.snd)
  exact h_computable
...
lemma evOutEq_decide_computable :
  ...
  have h_eq : PrimrecPred (fun q : Option BitString × BitString ↦ q.1 = some q.2) := by
    convert PrimrecRel.comp Primrec.eq ( Primrec.fst ) ( Primrec.option_some.comp ( Primrec.snd ) ) using 1;
  have h_eq : Computable (fun q : Option BitString × BitString ↦ decide (q.1 = some q.2)) := by
    obtain ⟨ f, hf ⟩ := h_eq;
    convert hf.to_comp;
  convert h_eq.comp ( Computable.pair ( evOut_computable.comp Computable.fst ) Computable.snd ) using 1
...
lemma truncGTerm_computable (hcomp : Computable
  ...
  have h_cond : Computable (fun q : ℕ × ℕ × BitString × BitString ↦ decide (cumNum approx q.1 (q.2.1 + 1) q.2.2.2 ≤ 2^(d + q.1) ∧ evOut q.2.1 = some q.2.2.1)) := by
    have h_cond : Computable (fun q : ℕ × ℕ × BitString × BitString ↦ decide (cumNum approx q.1 (q.2.1 + 1) q.2.2.2 ≤ 2^(d + q.1))) := by
      have h_cond : Computable (fun q : ℕ × ℕ × BitString ↦ cumNum approx q.1 q.2.1 q.2.2) := by
        convert cumNum_computable hcomp using 1;
      have h_cond : Computable (fun q : ℕ × ℕ × BitString ↦ decide (cumNum approx q.1 q.2.1 q.2.2 ≤ 2^(d + q.1))) := by
        have h_cond : Computable (fun q : ℕ × ℕ × BitString ↦ cumNum approx q.1 q.2.1 q.2.2) ∧ Computable (fun q : ℕ × ℕ × BitString ↦ 2^(d + q.1)) := by
          refine ⟨ h_cond, ?_ ⟩;
          convert Primrec.to_comp ( show Primrec ( fun q : ℕ × ℕ × BitString ↦ 2 ^ ( d + q.1 ) ) from ?_ ) using 1;
          convert Primrec.comp ( show Primrec ( fun n : ℕ ↦ 2 ^ n ) from ?_ ) ( show Primrec ( fun q : ℕ × ℕ × BitString ↦ d + q.1 ) from ?_ ) using 1;
          · convert primrec_two_pow_aux using 1;
          · exact Primrec.nat_add.comp ( Primrec.const d ) ( Primrec.fst );
        have h_cond : Computable (fun q : ℕ × ℕ × BitString ↦ (cumNum approx q.1 q.2.1 q.2.2, 2^(d + q.1))) := by
        ...
        have h_cond : Computable (fun q : ℕ × ℕ ↦ decide (q.1 ≤ q.2)) := by
          ...
          simp +decide [ PrimrecRel ];
          grind +suggestions;
        convert h_cond.comp ‹Computable fun q : ℕ × ℕ × BitString ↦ ( cumNum approx q.1 q.2.1 q.2.2, 2 ^ ( d + q.1 ) ) › using 1;
      convert h_cond.comp ( show Computable ( fun q : ℕ × ℕ × BitString × BitString ↦ ( q.1, q.2.1 + 1, q.2.2.2 ) ) from ?_ ) using 1;
      exact Computable.pair ( Computable.fst ) ( Computable.pair ( Computable.succ.comp ( Computable.fst.comp ( Computable.snd ) ) ) ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd ) ) ) );
    have h_cond : Computable (fun q : ℕ × ℕ × BitString × BitString ↦ decide (evOut q.2.1 = some q.2.2.1)) := by
      have h_cond : Computable (fun p : ℕ × BitString ↦ decide (evOut p.1 = some p.2)) := by
        exact evOutEq_decide_computable;
      convert h_cond.comp ( Computable.fst.comp ( Computable.snd ) |> Computable.pair <| Computable.fst.comp ( Computable.snd.comp ( Computable.snd ) ) ) using 1;
    rename_i h;
    convert Computable.cond h ( h_cond ) ( Computable.const false ) using 1;
    grind;
  have h_then : Computable (fun q : ℕ × ℕ × BitString × BitString ↦ evNum approx q.2.1 q.2.2.2 * 2^(q.1 - evK q.2.1)) := by
    have h_then : Computable (fun q : ℕ × ℕ × BitString × BitString ↦ evNum approx q.2.1 q.2.2.2) := by
      convert evNum_computable hcomp |> Computable.comp <| Computable.fst.comp ( Computable.snd ) |> Computable.pair <| Computable.snd.comp ( Computable.snd.comp ( Computable.snd ) ) using 1;
    have h_exp : Computable (fun q : ℕ × ℕ × BitString × BitString ↦ 2 ^ (q.1 - evK q.2.1)) := by
      have h_exp : Computable (fun q : ℕ × ℕ × BitString × BitString ↦ q.1 - evK q.2.1) := by
        ...
        convert h_exp.comp ( Computable.fst.pair ( evK_computable.comp ( Computable.fst.comp ( Computable.snd ) ) ) ) using 1;
    ...
    have h_mul : Computable₂ (fun a b : ℕ ↦ a * b) :=
      Primrec.nat_mul.to_comp
    have := Computable₂.comp h_mul h_then h_exp
    exact this
  have hreshape : Computable (fun q : (ℕ × BitString × BitString) × ℕ ↦
      (q.1.1, q.2, q.1.2.1, q.1.2.2)) :=
  ...
  have hcond' := h_cond.comp hreshape
  have hthen' := h_then.comp hreshape
  have hcombined := Computable.cond hcond' hthen' ( Computable.const 0 )
  exact hcombined.of_eq (fun q ↦ by
    by_cases h : cumNum approx q.1.1 (q.2 + 1) q.1.2.2 ≤ 2 ^ (d + q.1.1) ∧ evOut q.2 = some q.1.2.1 <;>
      simp [h])
...
lemma truncGapprox_computable (hcomp : Computable
  ...
  have hg : Computable₂ (fun (a : ℕ × BitString × BitString) (t : ℕ) ↦
      if cumNum approx a.1 (t + 1) a.2.2 ≤ 2 ^ (d + a.1) ∧ evOut t = some a.2.1 then
        evNum approx t a.2.2 * 2 ^ (a.1 - evK t)
      else 0) :=
    (truncGTerm_computable hcomp d).of_eq (fun _ ↦ rfl)
  exact (computable_range_sum _ hg (fun a ↦ a.1) Computable.fst).of_eq (fun _ ↦ rfl)
...
lemma incNum_computable_uniform (b : ℕ → ℕ → BitString → BitString → ℕ)
  ...
  · convert hb.comp ( Computable.fst.pair ( Computable.const 0 |> Computable.pair <| Computable.fst.comp ( Computable.snd.comp Computable.snd ) |> Computable.pair <| Computable.snd.comp ( Computable.snd.comp Computable.snd ) ) ) using 1;
  · apply Computable.of_eq;
    rotate_right;
    exact fun p ↦ b p.1.1 ( p.2 + 1 ) p.1.2.2.1 p.1.2.2.2 - 2 * b p.1.1 p.2 p.1.2.2.1 p.1.2.2.2;
    · have h_sub : Computable (fun p : ℕ × ℕ × BitString × BitString ↦ b p.1 (p.2.1 + 1) p.2.2.1 p.2.2.2) := by
        convert hb.comp _ using 1;
        rotate_left;
        exact fun p ↦ ( p.1, p.2.1 + 1, p.2.2.1, p.2.2.2 );
        · exact Computable.pair ( Computable.fst ) ( Computable.pair ( Computable.succ.comp ( Computable.fst.comp ( Computable.snd ) ) ) ( Computable.pair ( Computable.fst.comp ( Computable.snd.comp ( Computable.snd ) ) ) ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd ) ) ) ) );
        · rfl;
      have h_mul : Computable (fun p : ℕ × ℕ × BitString × BitString ↦ 2 * b p.1 p.2.1 p.2.2.1 p.2.2.2) := by
      ...
      have h_sub : Computable (fun p : ℕ × ℕ × BitString × BitString ↦ b p.1 (p.2.1 + 1) p.2.2.1 p.2.2.2 - 2 * b p.1 p.2.1 p.2.2.1 p.2.2.2) := by
        have h_sub : Computable (fun p : ℕ × ℕ ↦ p.1 - p.2) := by
          -- The subtraction function is primitive recursive, hence computable.
          have h_sub_primrec : Primrec (fun p : ℕ × ℕ ↦ p.1 - p.2) := by
            exact Primrec.nat_sub.comp ( Primrec.fst ) ( Primrec.snd );
          grind +suggestions;
        convert h_sub.comp ( Computable.pair ‹Computable fun p : ℕ × ℕ × BitString × BitString ↦ b p.1 ( p.2.1 + 1 ) p.2.2.1 p.2.2.2› ‹Computable fun p : ℕ × ℕ × BitString × BitString ↦ 2 * b p.1 p.2.1 p.2.2.1 p.2.2.2› ) using 1;
      convert h_sub.comp _ using 1;
      rotate_left;
      exact fun p ↦ ( p.1.1, p.2, p.1.2.2.1, p.1.2.2.2 );
      · exact Computable.pair ( Computable.fst.comp Computable.fst ) ( Computable.pair ( Computable.snd ) ( Computable.pair ( Computable.fst.comp ( Computable.snd.comp ( Computable.snd.comp Computable.fst ) ) ) ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd.comp Computable.fst ) ) ) ) );
      · rfl;
    · grind;
...
lemma evNum_computable_uniform (b : ℕ → ℕ → BitString → BitString → ℕ)
  ...
  · have h_comp : Computable (fun p : Option BitString ↦ p.isSome) := by
    ...
    exact h_comp.comp ( Computable.fst.comp ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd ) ) ) );
  · have hget : Computable (fun p : ℕ × ℕ × BitString × Option BitString × ℕ ↦
        p.2.2.2.1.get!) :=
      (Computable.option_getD
        (Computable.fst.comp ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd ) ) ))
        (Computable.const default)).of_eq (fun _ ↦ rfl)
    have hmap : Computable (fun p : ℕ × ℕ × BitString × Option BitString × ℕ ↦
        (p.1, p.2.2.2.2, p.2.2.2.1.get!, p.2.2.1)) :=
      Computable.pair Computable.fst
        (Computable.pair
          (Computable.snd.comp ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd ) ) ))
          (Computable.pair hget
            (Computable.fst.comp ( Computable.snd.comp ( Computable.snd )))))
    exact ((incNum_computable_uniform b hb).comp hmap).of_eq (fun _ ↦ rfl)
  · exact Computable.const 0;
  ...
  · ext; simp [evNum];
    cases evOut ‹ℕ × ℕ × BitString›.2.1 <;> simp +decide [ * ]
...
lemma cumNum_computable_uniform (b : ℕ → ℕ → BitString → BitString → ℕ)
  ...
  have h_g : Computable (fun q : (ℕ × ℕ × Nat × BitString) × ℕ ↦ evNum (b q.1.1) q.2 q.1.2.2.2 * 2 ^ (q.1.2.1 - evK q.2)) := by
    apply Computable.of_eq;
    rotate_right;
    exact fun p ↦ evNum ( b p.1.1 ) p.2 p.1.2.2.2 * 2 ^ ( p.1.2.1 - evK p.2 );
    · have h_term_computable : Computable (fun p : (ℕ × ℕ × Nat × BitString) × ℕ ↦ evNum (b p.1.1) p.2 p.1.2.2.2) := by
        convert evNum_computable_uniform b hb |> Computable.comp <| show Computable ( fun p : ( ℕ × ℕ × Nat × BitString ) × ℕ ↦ ( p.1.1, p.2, p.1.2.2.2 ) ) from ?_ using 1;
        apply Computable.pair;
        · exact Computable.fst.comp Computable.fst;
        · apply Computable.pair;
          · exact Computable.snd;
          · exact Computable.snd.comp ( Computable.snd.comp ( Computable.snd.comp Computable.fst ) );
      have h_exp_computable : Computable (fun p : (ℕ × ℕ × Nat × BitString) × ℕ ↦ p.1.2.1 - evK p.2) := by
        convert Primrec.to_comp _;
        exact Primrec.nat_sub.comp ( Primrec.fst.comp ( Primrec.snd.comp ( Primrec.fst ) ) ) ( Primrec.snd.comp ( Primrec.unpair.comp ( Primrec.snd ) ) );
      apply Computable.comp (Primrec.nat_mul.to_comp) (h_term_computable.pair (Computable.comp (primrec_two_pow_aux.to_comp) h_exp_computable));
    · exact fun _ ↦ rfl;
  have hg2 : Computable₂ (fun (a : ℕ × ℕ × ℕ × BitString) (i : ℕ) ↦
      evNum (b a.1) i a.2.2.2 * 2 ^ (a.2.1 - evK i)) := h_g.of_eq (fun _ ↦ rfl)
  exact (computable_range_sum _ hg2 (fun a ↦ a.2.2.1)
    (Computable.fst.comp ( Computable.snd.comp ( Computable.snd )))).of_eq (fun _ ↦ rfl)
...
lemma truncGTerm_computable_uniform (b : ℕ → ℕ → BitString → BitString → ℕ)
  ...
  rotate_left;
  exact fun q ↦ decide ( cumNum ( b q.1.1 ) q.1.2.1 ( q.2 + 1 ) q.1.2.2.2 ≤ 2 ^ ( d + q.1.2.1 ) ∧ evOut q.2 = some q.1.2.2.1 );
  · convert Computable.cond _ _ _ using 1;
    ...
    · have h_cumNum_computable : Computable (fun q : (ℕ × ℕ × BitString × BitString) × ℕ ↦ cumNum (b q.1.1) q.1.2.1 (q.2 + 1) q.1.2.2.2) := by
        convert cumNum_computable_uniform b hb |> Computable.comp <| _ using 1;
        rotate_left;
        exact fun q ↦ ( q.1.1, q.1.2.1, q.2 + 1, q.1.2.2.2 );
        · exact Computable.pair ( Computable.fst.comp Computable.fst ) ( Computable.pair ( Computable.fst.comp ( Computable.snd.comp Computable.fst ) ) ( Computable.pair ( Computable.succ.comp Computable.snd ) ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd.comp Computable.fst ) ) ) ) );
        · rfl;
      have h_exp_computable : Computable (fun q : (ℕ × ℕ × BitString × BitString) × ℕ ↦ 2 ^ (d + q.1.2.1)) := by
      ...
      have h_decide_computable : Computable (fun q : ℕ × ℕ ↦ decide (q.1 ≤ q.2)) := by
        ...
        · obtain ⟨ x, hx ⟩ := h;
          ...
          convert rfl;
      convert h_decide_computable.comp ( Computable.pair h_cumNum_computable h_exp_computable ) using 1;
    · convert evOutEq_decide_computable.comp _ using 1;
      rotate_left;
      exact fun q ↦ ( q.2, q.1.2.2.1 );
      · exact Computable.pair ( Computable.snd ) ( Computable.fst.comp ( Computable.snd.comp ( Computable.snd.comp ( Computable.fst ) ) ) );
      · rfl;
    · exact Computable.const false;
    · grind;
  · convert Computable.comp ( Primrec.nat_mul.to_comp ) ( Computable.pair _ _ ) using 1;
    · convert evNum_computable_uniform b hb |> Computable.comp <| Computable.pair ( Computable.fst.comp Computable.fst ) ( Computable.pair Computable.snd ( Computable.snd.comp ( Computable.snd.comp ( Computable.snd.comp Computable.fst ) ) ) ) using 1;
...
lemma truncGapprox_computable_uniform (b : ℕ → ℕ → BitString → BitString → ℕ)
  ...
  have hg : Computable₂ (fun (a : ℕ × ℕ × BitString × BitString) (t : ℕ) ↦
      if cumNum (b a.1) a.2.1 (t + 1) a.2.2.2 ≤ 2 ^ (d + a.2.1) ∧ evOut t = some a.2.2.1 then
        evNum (b a.1) t a.2.2.2 * 2 ^ (a.2.1 - evK t)
      else 0) :=
    (truncGTerm_computable_uniform b hb d).of_eq (fun _ ↦ rfl)
  exact (computable_range_sum _ hg (fun a ↦ a.2.1)
...
lemma tsum_evVal_out
  ...
  unfold evVal; simp +decide only [evK, Nat.unpair_pair]
  unfold evNum; simp +decide only [evOut, Nat.unpair_pair, Encodable.decode₂_encode, evK]
...
lemma extract_request_stream {f : BitString → BitString → ℝ≥0∞}
  ...
  · intro ctx;
    ...
    have h_reindex : (∑' n : ℕ, (if (Nat.unpair n).2 < evNum approx (Nat.unpair n).1 ctx then (2⁻¹ : ℝ≥0∞) ^ (evK (Nat.unpair n).1) else 0)) = (∑' t : ℕ, (∑' j : ℕ, (if j < evNum approx t ctx then (2⁻¹ : ℝ≥0∞) ^ (evK t) else 0))) := by
      ...
      rw [ ← Equiv.tsum_eq ( Equiv.ofBijective ( fun n : ℕ ↦ ( Nat.unpair n |>.1, Nat.unpair n |>.2 ) ) ⟨ fun a ↦ ?_, fun a ↦ ?_ ⟩ ) ];
      congr! 1;
      · exact fun n hn ↦ by simpa using congr_arg ( fun p ↦ Nat.pair p.1 p.2 ) hn;
      · exact ⟨ Nat.pair a.1 a.2, by simp +decide ⟩;
    have h_eq_lhs : (∑' n : ℕ, match (if ( Nat.unpair n ).2 < evNum approx ( Nat.unpair n ).1 ctx then ( evOut ( Nat.unpair n ).1 ).map fun o ↦ ( o, evK ( Nat.unpair n ).1 ) else none) with | some (_, l) => (2 : ℝ≥0∞)⁻¹ ^ l | none => 0)
        = (∑' n : ℕ, (if (Nat.unpair n).2 < evNum approx (Nat.unpair n).1 ctx then (2⁻¹ : ℝ≥0∞) ^ (evK (Nat.unpair n).1) else 0)) := by
      refine tsum_congr fun n ↦ ?_;
      unfold evNum; aesop;
    rw [h_eq_lhs, h_reindex]
    refine le_trans ?_ (h_sum ctx)
    refine le_of_eq ?_
    rw [← tsum_evVal hmono.1 hmono.2.1 ctx]
    refine tsum_congr fun t ↦ ?_;
    rw [ tsum_eq_sum ];
    any_goals exact Finset.range ( evNum approx t ctx );
    · simp +decide [ Finset.sum_ite, evVal ];
      rw [ Finset.filter_true_of_mem fun x hx ↦ Finset.mem_range.mp hx ]; norm_num [ dyadicValue ];
      rw [ div_eq_mul_inv, ENNReal.inv_pow ];
    · intro b hb; simp only [Finset.mem_range, not_lt] at hb; simp [Nat.not_lt.mpr hb];
  · intro ctx out
    have h_sum_eq : (∑' n : ℕ, (if (Nat.unpair n).2 < evNum approx (Nat.unpair n).1 ctx ∧ evOut (Nat.unpair n).1 = some out then (2 : ℝ≥0∞)⁻¹ ^ evK (Nat.unpair n).1 else 0)) = f out ctx := by
      ...
      have h_sum_eq : ∀ t : ℕ, ∑' j : ℕ, (if j < evNum approx t ctx ∧ evOut t = some out then (2 : ℝ≥0∞)⁻¹ ^ (evK t) else 0) = if evOut t = some out then evVal approx t ctx else 0 := by
        intro t; split_ifs <;> simp_all +decide [ dyadicValue, evVal ];
        rw [ tsum_eq_sum ];
        any_goals exact Finset.range ( evNum approx t ctx );
        · rw [ Finset.sum_congr rfl fun x hx ↦ if_pos <| Finset.mem_range.mp hx ]; norm_num [ div_eq_mul_inv ];
          norm_num [ ENNReal.inv_pow ];
        · grind;
      rw [ ← funext h_sum_eq ];
      rw [ ← ENNReal.tsum_prod ];
      rw [ ← Equiv.tsum_eq ( Equiv.ofBijective ( fun n : ℕ ↦ ( Nat.unpair n |>.1, Nat.unpair n |>.2 ) ) ⟨ fun n ↦ ?_, fun n ↦ ?_ ⟩ ) ];
      congr! 1;
      · exact fun m hm ↦ by simpa using congr_arg ( fun p ↦ Nat.pair p.1 p.2 ) hm;
      · exact ⟨ Nat.pair n.1 n.2, by simp +decide ⟩;
    convert h_sum_eq using 3;
...
lemma construct_out_partrec (req : BitString → ℕ → Option (BitString × ℕ))
  ...
  have h_partrec : Computable (fun p : (BitString × BitString) × ℕ ↦ Option.map Prod.fst (req p.1.2 p.2)) :=
    h_if_computable.comp ( hreqcomp.comp ( Computable.snd.comp ( Computable.fst ) |> Computable.pair <| Computable.snd ) )
  have hp := Computable.ofOption h_partrec
  change Partrec (fun p : (BitString × BitString) × ℕ ↦
    (Part.ofOption ((req p.1.2 p.2).map Prod.fst)))
  exact hp.of_eq (fun _ ↦ rfl)
...
lemma construct_prefix_machine (req : BitString → ℕ → Option (BitString × ℕ))
  ...
  · refine Partrec.bind ?_ ?_;
    · have hpred : Partrec₂ (fun (a : BitString × BitString) (n : ℕ) ↦
          (Part.some (decide (alloc a.2 n = some a.1 ∧ (req a.2 n).isSome = true)))) :=
        (Computable.partrec (construct_pred_computable req alloc hreqcomp hcomp)).of_eq (fun _ ↦ rfl)
...
/-- Body of the geometric requirement computation for Kraft-Chaitin allocator. -/
...
/-- Computes the requested node and length for the Kraft-Chaitin geometric requirement. -/
...
lemma geomReq_pair (approx : ℕ → BitString → BitString → ℕ) (ctx o : BitString)
  ...
  unfold geomReq
  simp +decide only [evOut, Nat.unpair_pair, Encodable.decode₂_encode, Option.bind_some]
  convert geomReqBody_eq approx _ using 1
  unfold evK; simp +decide only [Nat.unpair_pair]
...
lemma geomCross_succ_of_geomCross {approx : ℕ → BitString → BitString → ℕ}
  ...
  -- From `hmono` (in dyadic form) derive the natural-number doubling `2 * approx s ≤ approx (s+1)`.
  have hdouble : 2 * approx s o ctx ≤ approx (s + 1) o ctx := by
    have hm := hmono s o ctx
    simp only [dyadicValue] at hm
    -- Move to `ℝ` where division is well-behaved.
    have hmr : (approx s o ctx : ℝ) / 2 ^ s ≤ (approx (s + 1) o ctx : ℝ) / 2 ^ (s + 1) := by
      have := (ENNReal.toReal_le_toReal (by
          simp [ENNReal.div_eq_top]) (by simp [ENNReal.div_eq_top])).mpr hm
      simpa [ENNReal.toReal_div] using this
    rw [div_le_div_iff₀ (by positivity) (by positivity)] at hmr
    -- hmr : ↑(approx s) * 2^(s+1) ≤ ↑(approx (s+1)) * 2^s
    have hfin : (2 * approx s o ctx : ℝ) ≤ (approx (s + 1) o ctx : ℝ) := by
      have hpos : (0 : ℝ) < 2 ^ s := by positivity
      rw [pow_succ] at hmr
      nlinarith [hmr, hpos]
    exact_mod_cast hfin
  -- Conclude the natural-number crossing at stage `s+1`.
  calc 2 ^ (s + 1) = 2 * 2 ^ s := by rw [pow_succ]; ring
    _ ≤ 2 * (approx s o ctx * 2 ^ m) := by
        exact Nat.mul_le_mul_left 2 hcross
    _ = (2 * approx s o ctx) * 2 ^ m := by ring
    _ ≤ approx (s + 1) o ctx * 2 ^ m := Nat.mul_le_mul_right _ hdouble
...
lemma geomReq_kraft_le_perOutput_sum {approx : ℕ → BitString → BitString → ℕ}
  ...
  have h_reindex : (∑' n : ℕ, (match geomReq approx ctx n with | some (_, l) => (2 : ℝ≥0∞)⁻¹ ^ l | none => 0)) = (∑' a : ℕ, ∑' s : ℕ, ∑' l : ℕ, (match geomReq approx ctx (Nat.pair (Nat.pair a s) l) with | some (_, l') => (2 : ℝ≥0∞)⁻¹ ^ l' | none => 0)) := by
    have h_reindex : ∀ (f : ℕ → ℝ≥0∞), (∑' n : ℕ, f n) = (∑' a : ℕ, ∑' s : ℕ, ∑' l : ℕ, f (Nat.pair (Nat.pair a s) l)) := by
      ...
      rw [ ← Equiv.tsum_eq ( Equiv.ofBijective ( fun p : ( ℕ × ℕ ) × ℕ ↦ Nat.pair ( Nat.pair p.1.1 p.1.2 ) p.2 ) ⟨ fun p ↦ ?_, fun p ↦ ?_ ⟩ ) ];
      congr! 1;
      · simp +decide [ Nat.pair_eq_pair ];
      · exact ⟨ ⟨ ⟨ Nat.unpair p |>.1 |> Nat.unpair |>.1, Nat.unpair p |>.1 |> Nat.unpair |>.2 ⟩, Nat.unpair p |>.2 ⟩, by simp +decide ⟩;
    exact h_reindex _;
  ...
  have h_restrict : (∑' a : ℕ, (∑' s : ℕ, (∑' l : ℕ, (match geomReq approx ctx (Nat.pair (Nat.pair a s) l) with | some (_, l') => (2 : ℝ≥0∞)⁻¹ ^ l' | none => 0)))) ≤ (∑' o : BitString, (∑' s : ℕ, (∑' l : ℕ, (match geomReq approx ctx (Nat.pair (Nat.pair (Encodable.encode o) s) l) with | some (_, l') => (2 : ℝ≥0∞)⁻¹ ^ l' | none => 0)))) := by
    have h_restrict : (∑' a : ℕ, (∑' s : ℕ, (∑' l : ℕ, (match geomReq approx ctx (Nat.pair (Nat.pair a s) l) with | some (_, l') => (2 : ℝ≥0∞)⁻¹ ^ l' | none => 0)))) = (∑' a : ℕ, if a ∈ Set.range (Encodable.encode : BitString → ℕ) then (∑' s : ℕ, (∑' l : ℕ, (match geomReq approx ctx (Nat.pair (Nat.pair a s) l) with | some (_, l') => (2 : ℝ≥0∞)⁻¹ ^ l' | none => 0))) else 0) := by
      congr;
      ext a; split_ifs <;> simp_all +decide [ geomReq ];
      ...
      obtain ⟨s, hs⟩ : ∃ s : ℕ, dyadicValue (approx s out ctx) s > 0 := by
        ...
        exact not_forall_not.mp fun h ↦ h_pos.ne' <| hsup out ctx ▸ by
          simp +decide [show ∀ s : ℕ, dyadicValue (approx s out ctx) s = 0 from
            fun s ↦ le_antisymm (le_of_not_gt fun hs ↦ h s hs)
              (show (0 : ℝ≥0∞) ≤ dyadicValue (approx s out ctx) s from zero_le)]
