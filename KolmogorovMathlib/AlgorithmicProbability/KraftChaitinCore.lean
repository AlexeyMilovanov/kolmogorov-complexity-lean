...
lemma iSup_dyadicValue_shift (approx : ℕ → BitString → BitString → ℕ) (d : ℕ)
  ...
  · intro i
    ...
    rw [ show ( 2 : ℝ≥0∞ ) ^ i =
        ( 2 : ℝ≥0∞ ) ^ d * ( 2 : ℝ≥0∞ ) ^ ( i - d ) by
      rw [ ← pow_add, Nat.add_sub_of_le ‹d ≤ i› ] ]
    ring_nf
    ...
    · convert mul_le_mul_right (hmono (s - d) out ctx) (2⁻¹ ^ d) using 1
      ...
      · have hs1' : s + 1 - d = s - d + 1 := by omega
        have h_eq : 2 ^ (s + 1) = (2 : ℝ≥0∞) ^ (s + 1 - d + d) :=
          congr_arg (fun x ↦ (2 : ℝ≥0∞) ^ x)
            (show s + 1 = s + 1 - d + d by omega)
        have h_inv : ((2 : ℝ≥0∞) ^ d)⁻¹ = 2⁻¹ ^ d := by rw [← ENNReal.inv_pow]
        unfold dyadicValue
        rw [h_eq, pow_add, div_eq_mul_inv, ENNReal.mul_inv]
        · rw [hs1', ← mul_assoc, mul_comm _ ((2 : ℝ≥0∞) ^ d)⁻¹, h_inv,
            ← div_eq_mul_inv]
        · exact Or.inl (by norm_num)
        · exact Or.inl (ENNReal.pow_ne_top (by norm_num))
...
lemma tsum_dyadicValue_incNum
  ...
  · intro i
    have h_partial_sum :
        ∑ k ∈ Finset.range i, dyadicValue (incNum approx k out ctx) k ≤
          dyadicValue (approx (i - 1) out ctx) (i - 1) := by
      rcases i with _ | n
      · simp
  ...
  · intro w hw
    rcases exists_lt_of_lt_ciSup hw with ⟨ s, hs ⟩
    use s + 1
...
lemma dyadicValue_truncGapprox (d S : ℕ) (out ctx : BitString) :
  ...
  · intro a ha₁ ha₂ ha₃
    ...
    rw [ ENNReal.div_le_iff_le_mul ] <;> norm_cast <;> norm_num [ pow_add ] at *
    linarith
  · intro b hb₁ hb₂ hb₃
    contrapose! hb₂
    simp_all only [truncCum]
    have h_div : (cumNum approx S (b + 1) ctx : ℝ≥0∞) / 2 ^ S > 2 ^ d := by
      rw [ gt_iff_lt, ENNReal.lt_div_iff_mul_lt ] <;> norm_cast <;>
        norm_num [ pow_add ] at *
      linarith
    refine lt_of_lt_of_le h_div ?_
    exact le_of_eq
      (dyadicValue_cumNum (approx := approx) S ( b + 1 ) ctx ( by linarith ))
  · intro a ha₁ ha₂ ha₃
    rw [ ← dyadicValue_evNum_scale a S ctx
      ( Nat.le_of_lt ( Nat.unpair_right_le a |> lt_of_le_of_lt <| ha₁ ) ) ]
    ring_nf
    unfold dyadicValue
    norm_num [ mul_assoc, mul_comm, mul_left_comm, pow_add ]
    rw [ ENNReal.div_eq_inv_mul ]
    ring
...
lemma truncGapprox_mono (d : ℕ) (S : ℕ) (out ctx : BitString) :
  ...
  exact Finset.sum_le_sum_of_subset_of_nonneg
    (Finset.range_mono (Nat.le_succ _)) fun _ _ _ ↦ by
  ...
  have h_sum_computable : ∀ (f : α → ℕ → ℕ), Computable₂ f → Computable
    ...
    have h_sum_computable : ∃ F : α → ℕ × ℕ → ℕ, Computable₂ F ∧ ∀ a n, F a
      ...
      · simp only [Computable₂] at *
        have h_sum_computable :
            Computable (fun p : α × ℕ × ℕ ↦ p.2.2 + f p.1 p.2.1) := by
...
lemma truncGTerm_computable (hcomp : Computable
  ...
  exact hcombined.of_eq (fun q ↦ by
    by_cases h : cumNum approx q.1.1 (q.2 + 1) q.1.2.2 ≤ 2 ^ (d + q.1.1) ∧
        evOut q.2 = some q.1.2.1 <;>
      simp [h])
...
lemma tsum_accepted_le (d : ℕ) (ctx : BitString) :
  ...
  have h_aterm : ∀ S, (∑ t ∈ Finset.range S, aterm t) ≤ (2 : ℝ≥0∞) ^ d := by
    ...
    | succ S ih =>
      simp_all only [Finset.sum_range_succ]
      by_cases h : truncCum approx ( S + 1 ) ctx ≤ 2 ^ d <;>
        simp_all only [not_le, truncCum];
      ...
      · simp only [aterm] at ih ⊢
        rw [ if_neg ] <;>
          simp_all only [Finset.sum_range_succ, add_zero, not_le, truncCum];
...
lemma tsum_evVal_out
  ...
  -- By definition of $evOut$, `evOut t = some out` iff
  -- `t = Nat.pair (Encodable.encode out) k` for some `k`.
  ...
  -- Reindex the sum over events for `out` by their stage `k`.
  have h_sum_eq : (∑' t, if evOut t = some out then evVal approx t ctx else 0) =
    ...
    erw [ ← Equiv.tsum_eq ( Equiv.ofBijective
      ( fun k : ℕ ↦
          ⟨ Nat.pair ( Encodable.encode out ) k, ⟨ k, rfl ⟩ ⟩ :
...
lemma tsum_evVal
  ...
  -- Swap the output/event sums, observing that each event contributes to at most one output.
...
This follows the truncation/`ev*` machinery so `extract_request_stream` can reuse it.
-/
...
lemma extract_request_stream {f : BitString → BitString → ℝ≥0∞}
  ...
  · intro ctx;
    ...
    have h_eq_lhs :
        (∑' n : ℕ, match
          (if ( Nat.unpair n ).2 < evNum approx ( Nat.unpair n ).1 ctx then
            ( evOut ( Nat.unpair n ).1 ).map fun o ↦
              ( o, evK ( Nat.unpair n ).1 )
          else none) with
          | some (_, l) => (2 : ℝ≥0∞)⁻¹ ^ l
          | none => 0) =
        (∑' n : ℕ, if (Nat.unpair n).2 < evNum approx (Nat.unpair n).1 ctx then
          (2⁻¹ : ℝ≥0∞) ^ (evK (Nat.unpair n).1) else 0) := by
    ...
    have h_sum_eq :
      ...
      have h_sum_eq : ∀ t : ℕ, ∑' j : ℕ,
          (if j < evNum approx t ctx ∧ evOut t = some out then
            (2 : ℝ≥0∞)⁻¹ ^ (evK t) else 0) =
          if evOut t = some out then evVal approx t ctx else 0 := by
        intro t
        split_ifs <;>
...
lemma construct_out_partrec (req : BitString → ℕ → Option (BitString × ℕ))
  ...
  have h_partrec : Computable
      (fun p : (BitString × BitString) × ℕ ↦ Option.map Prod.fst (req p.1.2 p.2)) :=
    h_if_computable.comp
      ( hreqcomp.comp
        ( Computable.snd.comp Computable.fst |> Computable.pair <| Computable.snd ) )
...
lemma construct_prefix_machine (req : BitString → ℕ → Option (BitString × ℕ))
  ...
  · refine Partrec.bind ?_ ?_;
    · have hpred : Partrec₂ (fun (a : BitString × BitString) (n : ℕ) ↦
        ...
        (Computable.partrec (construct_pred_computable req alloc hreqcomp hcomp)).of_eq
          (fun _ ↦ rfl)
    ...
    have h_sum : (∑' l : ℕ, if L ≤ l then (2 : ℝ≥0∞)⁻¹ ^ l else 0) ≤ f o ctx := by
      have h_sum : (∑' l : ℕ, if L ≤ l then (2 : ℝ≥0∞)⁻¹ ^ l else 0) = (2 : ℝ≥0∞)⁻¹ ^ (L - 1) := by
        have h_sum : (∑' l : ℕ, if L ≤ l then (2 : ℝ≥0∞)⁻¹ ^ l else 0) =
          ...
          · -- Adding `L` to `x.val - L = y.val - L` gives `x.val = y.val`.
            ...
            have h_eq : x.val = y.val := by
              linarith [
                Nat.sub_add_cancel ( show L ≤ x.val from by aesop ),
                Nat.sub_add_cancel ( show L ≤ y.val from by aesop ) ]
        ...
        · simp_all +decide only [zero_tsub, pow_zero, mul_one, not_lt_zero,
            not_isEmpty_of_nonempty, IsEmpty.forall_iff, implies_true, and_true,
            false_and]
        ...
        exact h.2.elim
          ...
          fun h ↦ h ( by
            exact Nat.le_induction ( by tauto )
              ( fun k hk ih ↦ by
                exact geomCross_succ_of_geomCross hmono o ctx l k ih ) _
...
lemma geomReq_geometric_bound {f : BitString → BitString → ℝ≥0∞}
  ...
  obtain ⟨l, hl⟩ : ∃ l : ℕ, 1 ≤ l ∧ ∃ s : ℕ, geomCross approx out ctx l s ∧ ∀ l' : ℕ, l' < l → ¬∃ s'
    ...
    have hL : ∃ l : ℕ, 1 ≤ l ∧ ∃ s : ℕ, geomCross approx out ctx l s := by
      -- Since `f out ctx > 0`, some stage has positive dyadic value.
    ...
    obtain ⟨l, hl⟩ : ∃ l : ℕ, 1 ≤ l ∧ ∃ s : ℕ, geomCross approx out ctx l s ∧ ∀ l' : ℕ, l' < l → ¬∃
      ...
      obtain ⟨l, hl⟩ : ∃ l : ℕ, l ∈ {l : ℕ | 1 ≤ l ∧ ∃ s : ℕ, geomCross approx out ctx l s} ∧ ∀ l' :
        ...
        have := h_well_founded.has_min
          { l | 1 ≤ l ∧ ∃ s, geomCross approx out ctx l s }
          ⟨ _, hL.choose_spec ⟩
        tauto
  ...
  have h_emitted :
      ⨆ n, (match geomReq approx ctx n with
        | some (o, l) => if o = out then (2 : ℝ≥0∞)⁻¹ ^ l else 0
        | none => 0) ≥ (2 : ℝ≥0∞)⁻¹ ^ l := by
    obtain ⟨s₀, hs₀⟩ : ∃ s₀ : ℕ, geomCross approx out ctx l s₀ ∧
        ∀ s' : ℕ, s' < s₀ → ¬geomCross approx out ctx l s' := by
