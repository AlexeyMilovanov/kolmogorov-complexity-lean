...
theorem bitsToNat_primrec : Primrec bitsToNat := by
  unfold bitsToNat
  have hstep : Primrec₂ (fun (_ : List Bool) (p : Bool × ℕ) ↦
      2 * p.2 + (if p.1 then 1 else 0)) := by
...
theorem mass_total (Q : CodedFiniteDistribution) :
  ...
  | cons e l ih =>
    ...
    rw [ show List.foldr ( fun e_1 acc ↦
      ( if e_1.point = e.point then e_1.mass.value else 0 ) + acc ) 0 l = 0 from ?_ ]
    · ring;
    have h_foldr_zero : ∀ {l : List CodedDistributionEntry},
      e.point ∉ List.foldr (fun e_1 acc ↦ insert e_1.point acc) Finset.empty l →
      List.foldr (fun e_1 acc ↦
        (if e_1.point = e.point then e_1.mass.value else 0) + acc) 0 l = 0 := by
...
theorem isValidCodeBool_primrec : Primrec isValidCodeBool := by
  have h_primrec : Primrec (fun w : BitString ↦
      codedDistributionDataCode (decodeDistributionData w)) :=
...
theorem isProbBool_primrec : Primrec isProbBool := by
  have totalMassRat_primrec : Primrec totalMassRat := by
    ...
    exact Primrec.list_foldr Primrec.id (Primrec.const RatMass.zero)
      (Primrec.comp h_add Primrec.snd).to₂
...
theorem coveredBool_primrec :
  ...
  have h1 : Primrec (fun p : (List BitString × ℕ) × BitString ↦ p.1.1) :=
    Primrec.fst.comp Primrec.fst
  have ha : Primrec₂ (fun (p : (List BitString × ℕ) × BitString) (w : BitString) ↦
  ...
  have hrange : Primrec (fun p : ((List BitString × ℕ) × BitString) × BitString ↦
      List.range (p.1.1.2 + 1)) :=
    Primrec.list_range.comp (Primrec.succ.comp (Primrec.snd.comp (Primrec.fst.comp Primrec.fst)))
  have hany : Primrec₂ (fun (p : ((List BitString × ℕ) × BitString) × BitString) (k : ℕ) ↦
      levelSetMemBool p.2 k p.1.2) := by
    have hw : Primrec (fun (p : (((List BitString × ℕ) × BitString) × BitString) × ℕ) ↦ p.1.2) :=
      Primrec.snd.comp Primrec.fst
    have hk : Primrec (fun (p : (((List BitString × ℕ) × BitString) × BitString) × ℕ) ↦ p.2) :=
      Primrec.snd
    have hx : Primrec (fun (p : (((List BitString × ℕ) × BitString) × BitString) × ℕ) ↦ p.1.1.2) :=
      Primrec.snd.comp (Primrec.fst.comp Primrec.fst)
    have hm : Primrec (fun (p : (((List BitString × ℕ) × BitString) × BitString) × ℕ) ↦
        ((p.1.2, p.2), p.1.1.2)) :=
      Primrec.pair (Primrec.pair hw hk) hx
    exact levelSetMemBool_primrec.comp hm
  have hb : Primrec₂ (fun (p : (List BitString × ℕ) × BitString) (w : BitString) ↦
      (List.range (p.1.2 + 1)).any (fun k ↦ levelSetMemBool w k p.2)) :=
...
theorem countHalts_primrec (c : Code) :
  ...
  have hp_lst : Primrec (fun (p : ℕ × ℕ) ↦ boundedPrograms p.1) :=
    primrec_boundedPrograms.comp Primrec.fst
  have hp_eval : Primrec (fun (a : (ℕ × ℕ) × BitString) ↦
      Code.evaln a.1.2 c (Encodable.encode (a.2, ([] : List BitString)))) :=
    (evaln_primrec c).comp (Primrec.snd.comp Primrec.fst)
      (Primrec.encode.comp (Primrec.pair Primrec.snd (Primrec.const [])))
  have hp_some : Primrec₂ (fun (p : ℕ × ℕ) (y : BitString) ↦
      (Code.evaln p.2 c (Encodable.encode (y, ([] : List BitString)))).isSome) :=
...
theorem exists_max_countHalts (c : Code) (alpha : ℕ) :
  ...
  obtain ⟨t, ht⟩ : ∃ t, t ∈ Set.range (fun t ↦ countHalts c alpha t) ∧
      ∀ t' ∈ Set.range (fun t ↦ countHalts c alpha t), t' ≤ t := by
...
theorem runOut_complete {c : Code} {U : Map} (hc : IsCodeFor c U) {p out : BitString}
  ...
  obtain ⟨t, ht⟩ : ∃ t, Encodable.encode out ∈
      Code.evaln t c (Encodable.encode ((p, []) : BitString × BitString)) := by
    have h_evaln : Encodable.encode out ∈
        c.eval (Encodable.encode ((p, []) : BitString × BitString)) := by
      simp_all +decide [ produces, IsCodeFor ];
    grind +suggestions;
  -- By definition of `runOut`, we have `runOut c t p = (Code.evaln t c (Encodable.encode
  -- ((p, []) : BitString × BitString))).bind (fun r ↦ (Encodable.decode r : Option BitString))`.
  use t
  simp [runOut];
...
theorem code_mem_snapshot_of_max {c : Code} {U : Map} (hc : IsCodeFor c U) (alpha t : ℕ)
  ...
  use t'
  simp_all +decide only [countHalts]
  ...
  obtain ⟨ a, ha₁, ha₂ ⟩ := ht'
  specialize @h_countP_mono ( boundedPrograms alpha ) p hp
  ...
  apply h_countP_mono
  exact (by
    unfold snapshotCodes at hmax
    simp_all +decide only [List.mem_filterMap, not_exists, not_and]
...
theorem exists_halting_program_of_complexity_le (U : Map) (P : CodedFiniteDistribution)
  ...
  · obtain ⟨p, hp_prod, hp_len⟩ : ∃ p, produces U p [] P.code ∧
        (programLength p : ENat) = KP U P.code [] :=
...
theorem selector_check_computable (c : Code) : Computable (fun st : BitString × ℕ ↦
  ...
  have h_count : Computable (fun st : BitString × ℕ ↦ countHalts c (selAlpha st.1) st.2) :=
    @Computable.comp (BitString × ℕ) (ℕ × ℕ) ℕ _ _ _
      ...
      (@Computable.pair (BitString × ℕ) ℕ ℕ _ _ _
        ...
        (@Computable.comp (BitString × ℕ) BitString ℕ _ _ _ selAlpha
          Prod.fst selAlpha_computable Computable.fst)
        Computable.snd)
  ...
  exact (@Computable.comp (BitString × ℕ) (ℕ × ℕ) Bool _ _ _
    ...
    eq_decide_computable (@Computable.pair (BitString × ℕ) ℕ ℕ _ _ _ _ _ h_count h_selH)).of_eq
      (fun _ ↦ rfl)

theorem selector_post_computable (c : Code) : Computable (fun st : BitString × ℕ ↦
    (allStrings (selNat st.1)).find?
  ...
  have h_covered : Primrec₂ (fun (st : BitString × ℕ) (x : BitString) ↦
      ! coveredBool (snapshotCodes c (selAlpha st.1) st.2) (selMaxK st.1) x) := by
    have hw : Primrec (fun p : (BitString × ℕ) × BitString ↦
        (snapshotCodes c (selAlpha p.1.1) p.1.2, selMaxK p.1.1)) :=
    ...
    have hp : Primrec (fun p : (BitString × ℕ) × BitString ↦
        ((snapshotCodes c (selAlpha p.1.1) p.1.2, selMaxK p.1.1), p.2)) :=
      Primrec.pair hw hx
...
theorem partrec_selectorFn (c : Code) : Partrec (selectorFn c) := by
  ...
  have hf : Partrec (fun s : BitString ↦
      Nat.rfind (fun t ↦ Part.some (decide (countHalts c (selAlpha s) t = selH s)))) :=
...
theorem complexity_decode_of_mem_snapshot {c : Code} {U : Map} (hc : IsCodeFor c U)
  ...
  -- By definition of `snapshotCodes`, there exists a program `p` with
  -- `p ∈ boundedPrograms alpha` and `runOut c t p = some w`.
...
theorem exists_partrec_uncovered_selector (U : Map) (hU : IsOptimalPrefixConditional U) :
  ...
  -- By definition of `IsOptimalPrefixConditional`, there exists a code `c`
  -- such that `IsCodeFor c U`.
  ...
  · intro n alpha max_k hcov
    ...
    obtain ⟨t₀, ht0, ht0_min⟩ : ∃ t₀, countHalts c alpha t₀ = h ∧
        ∀ m < t₀, countHalts c alpha m ≠ h := by
    ...
    have hpred : pred x0 = true := by
      by_contra h_contra
      obtain ⟨w, hw⟩ : ∃ w ∈ snap, isValidCodeBool w = true ∧ isProbBool w = true ∧
          ∃ k ≤ max_k, levelSetMemBool w k x0 = true := by
        grind +locals;
