...
theorem mass_le_one_of_isProbability (P : CodedFiniteDistribution)
  ...
  · calc P.mass x ≤ ∑ y ∈ P.support, P.mass y :=
          Finset.single_le_sum (f := fun y => P.mass y) (fun _ _ => zero_le _) hx
      _ = 1 := hP
  · rw [CodedFiniteDistribution.mass_eq_zero_of_not_mem_support P x hx]; exact zero_le _
...
theorem levelSet_level_bound (U : Map) (hU : IsOptimalPrefixConditional U) :
  ...
  have hM : KP U x P.code ≤ M := by
    refine le_trans (hc₁ _ _) ?_;
    convert add_le_add_right ( hc₂ x ) ( c₁ : ENat ) using 1 ; simp +arith +decide;
    · exact add_comm _ _;
    · norm_cast ; simp +arith +decide [ hn ] ; ring!;
  have h_exp : (2 : ℝ≥0∞)⁻¹ ^ M ≤ (2 : ℝ≥0∞) ^ beta * P.mass x := by
    refine le_trans ?_ (hdef.trans ?_);
    · convert complexityWeight_le_of_le hM using 1;
    · norm_num;
