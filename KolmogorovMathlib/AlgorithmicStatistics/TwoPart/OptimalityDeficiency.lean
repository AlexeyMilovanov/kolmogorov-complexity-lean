...
theorem complexityWeight_add (a b : ENat) :
  ...
  | coe k =>
      ...
      | coe m =>
          rw [← Nat.cast_add, complexityWeight_coe, complexityWeight_coe, complexityWeight_coe, pow_add]
...
theorem randomness_optimality (U : Map) (hU : IsOptimalPrefixConditional U) :
  ...
  have h1 : KPPlain U x ≤ KPPlain U P.code + KP U x P.code + (c_upper + c_right : ℕ) := by
    calc
      ...
      _ = KPPlain U P.code + KP U x P.code + (c_upper + c_right : ℕ) := by
        push_cast
        abel
  have h2 : complexityWeight (KPPlain U P.code + KP U x P.code + (c_upper + c_right : ℕ)) ≤ complexityWeight (KPPlain U x) :=
  ...
  have h3 : complexityWeight (KPPlain U P.code) * complexityWeight (KP U x P.code) * (2 : ℝ≥0∞)⁻¹ ^ (c_upper + c_right) ≤
  ...
  have h4 : complexityWeight (KP U x P.code) * (2 : ℝ≥0∞)⁻¹ ^ (c_upper + c_right) ≤ (2 : ℝ≥0∞) ^ beta * P.mass x := by
    -- `CW(P) * (CW(x|P) * 2^{-c}) ≤ CW(P) * (2^beta * P(x))`
    have h3_rewrite : complexityWeight (KPPlain U P.code) * (complexityWeight (KP U x P.code) * (2 : ℝ≥0∞)⁻¹ ^ (c_upper + c_right)) ≤
        complexityWeight (KPPlain U P.code) * ((2 : ℝ≥0∞) ^ beta * P.mass x) := by
      calc
        complexityWeight (KPPlain U P.code) * (complexityWeight (KP U x P.code) * (2 : ℝ≥0∞)⁻¹ ^ (c_upper + c_right))
            = complexityWeight (KPPlain U P.code) * complexityWeight (KP U x P.code) * (2 : ℝ≥0∞)⁻¹ ^ (c_upper + c_right) := by rw [mul_assoc]
  ...
  calc
    complexityWeight (KP U x P.code)
        = complexityWeight (KP U x P.code) * (2 : ℝ≥0∞)⁻¹ ^ (c_upper + c_right) * (2 : ℝ≥0∞) ^ (c_upper + c_right) := by
          rw [mul_assoc, ← ENNReal.inv_pow, ENNReal.inv_mul_cancel (by norm_num) (by norm_num), mul_one]
...
theorem optimalityDeficiency_of_randomnessDeficiency (U : Map)
  ...
  have h3 : complexityWeight (KPPlain U x) ≤ (2 : ℝ≥0∞) ^ (beta + c1) * P.mass x := by
    calc
      complexityWeight (KPPlain U x) = complexityWeight (KPPlain U x) * (2 : ℝ≥0∞)⁻¹ ^ c1 * (2 : ℝ≥0∞) ^ c1 := by
        rw [mul_assoc, ← ENNReal.inv_pow, ENNReal.inv_mul_cancel (by norm_num) (by norm_num), mul_one]
  ...
  calc
    ...
    _ ≤ ((2 : ℝ≥0∞) ^ (beta + c1) * P.mass x) * (complexityWeight (P.complexity U) * (2 : ℝ≥0∞) ^ alpha) := by
      gcongr
