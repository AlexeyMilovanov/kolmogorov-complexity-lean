...
noncomputable def OptimalityDeficiencyLe (U : Map) (P : CodedFiniteDistribution)
    (x : BitString) (beta : Nat) : Prop :=
  complexityWeight (KPPlain U x) ≤
    (2 : ENNReal) ^ beta * (complexityWeight (P.complexity U) * P.mass x)

/-- Optimality deficiency for a finite-set uniform model. -/
noncomputable def SetOptimalityDeficiencyLe (U : Map) (S : Finset BitString)
    (hS : S.Nonempty) (x : BitString) (beta : Nat) : Prop :=
  OptimalityDeficiencyLe U (codedUniformOn S hS) x beta

/-- The tight realized finite-set optimality gap. -/
def RealizedSetOptimalityGap (U : Map) (A : Finset BitString) (hA : A.Nonempty)
    (x : BitString) (delta i j kx : ℕ) : Prop :=
  x ∈ A ∧
...
noncomputable def IsOptimalStochastic (U : Map) (x : BitString)
    (alpha beta : Nat) : Prop :=
  ∃ P : CodedFiniteDistribution,
    P.IsProbability ∧ P.complexity U ≤ (alpha : ENat) ∧
      OptimalityDeficiencyLe U P x beta

/-- The `x` is `(alpha, beta)`-stochastic via a set model. -/
noncomputable def IsOptimalSetStochastic (U : Map) (x : BitString)
    (alpha beta : Nat) : Prop :=
  ∃ (S : Finset BitString) (hS : S.Nonempty),
    x ∈ S ∧ setComplexity U S hS ≤ (alpha : ENat) ∧
      SetOptimalityDeficiencyLe U S hS x beta

/-! ### Monotonicity lemmas -/

theorem OptimalityDeficiencyLe.mono_beta {U : Map} {P : CodedFiniteDistribution}
    {x : BitString} {beta beta' : Nat} (h : beta ≤ beta')
    (hdef : OptimalityDeficiencyLe U P x beta) :
...
theorem SetOptimalityDeficiencyLe.mono_beta {U : Map} {S : Finset BitString}
    {hS : S.Nonempty} {x : BitString} {beta beta' : Nat} (h : beta ≤ beta')
    (hdef : SetOptimalityDeficiencyLe U S hS x beta) :
    SetOptimalityDeficiencyLe U S hS x beta' :=
  OptimalityDeficiencyLe.mono_beta h hdef

theorem IsIJDescription.mono_i {U : Map} {x : BitString} {S : Finset BitString}
    {hS : S.Nonempty} {i i' j : Nat} (h : i ≤ i')
    (hdesc : IsIJDescription U x S hS i j) :
    IsIJDescription U x S hS i' j := by
  unfold IsIJDescription at *
  rcases hdesc with ⟨hx, hcomp, hcard⟩
  exact ⟨hx, hcomp.trans (by exact_mod_cast h), hcard⟩

theorem IsIJDescription.mono_j {U : Map} {x : BitString} {S : Finset BitString}
    {hS : S.Nonempty} {i j j' : Nat} (h : j ≤ j')
    (hdesc : IsIJDescription U x S hS i j) :
    IsIJDescription U x S hS i j' := by
  unfold IsIJDescription at *
  rcases hdesc with ⟨hx, hcomp, hcard⟩
...
theorem InDescriptionProfile.mono_i {U : Map} {x : BitString} {i i' j : Nat}
    (h : i ≤ i') (hprof : InDescriptionProfile U x i j) :
    InDescriptionProfile U x i' j := by
  rcases hprof with ⟨S, hS, hdesc⟩
  exact ⟨S, hS, hdesc.mono_i h⟩

theorem InDescriptionProfile.mono_j {U : Map} {x : BitString} {i j j' : Nat}
    (h : j ≤ j') (hprof : InDescriptionProfile U x i j) :
    InDescriptionProfile U x i j' := by
  rcases hprof with ⟨S, hS, hdesc⟩
  exact ⟨S, hS, hdesc.mono_j h⟩

theorem IsOptimalStochastic.mono_alpha {U : Map} {x : BitString}
    {alpha alpha' beta : Nat} (h : alpha ≤ alpha')
    (hstoch : IsOptimalStochastic U x alpha beta) :
    IsOptimalStochastic U x alpha' beta := by
  rcases hstoch with ⟨P, hP, hcomp, hdef⟩
  exact ⟨P, hP, hcomp.trans (by exact_mod_cast h), hdef⟩

theorem IsOptimalStochastic.mono_beta {U : Map} {x : BitString}
    {alpha beta beta' : Nat} (h : beta ≤ beta')
    (hstoch : IsOptimalStochastic U x alpha beta) :
    IsOptimalStochastic U x alpha beta' := by
  rcases hstoch with ⟨P, hP, hcomp, hdef⟩
  exact ⟨P, hP, hcomp, hdef.mono_beta h⟩

theorem IsOptimalSetStochastic.mono_alpha {U : Map} {x : BitString}
    {alpha alpha' beta : Nat} (h : alpha ≤ alpha')
    (hstoch : IsOptimalSetStochastic U x alpha beta) :
    IsOptimalSetStochastic U x alpha' beta := by
  rcases hstoch with ⟨S, hS, hx, hcomp, hdef⟩
  exact ⟨S, hS, hx, hcomp.trans (by exact_mod_cast h), hdef⟩

theorem IsOptimalSetStochastic.mono_beta {U : Map} {x : BitString}
    {alpha beta beta' : Nat} (h : beta ≤ beta')
    (hstoch : IsOptimalSetStochastic U x alpha beta) :
    IsOptimalSetStochastic U x alpha beta' := by
  rcases hstoch with ⟨S, hS, hx, hcomp, hdef⟩
...
theorem setOptimalityDeficiencyLe_iff_of_mem {U : Map} {S : Finset BitString}
    {hS : S.Nonempty} {x : BitString} (hx : x ∈ S) {beta : Nat} :
    SetOptimalityDeficiencyLe U S hS x beta ↔
...
theorem inDescriptionProfile_lengthUniform {U : Map} {x : BitString} {i : Nat}
    (h_comp : setComplexity U (stringsOfLength x.length)
      (codedStringsOfLength_nonempty x.length) ≤ (i : ENat)) :
