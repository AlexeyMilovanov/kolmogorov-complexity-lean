...
theorem isCanonicalUniformCodeBool_iff (c : BitString) :
  ...
  · -- Let `S := ((decodeDistributionData c).map CodedDistributionEntry.point).toFinset`. From `h` (unfolded), we have `hSne : S.Nonempty` and `c = codedDistributionDataCode ((canonicalFinsetList S).map (fun x => {point := x, mass := ratMassInvNat (max 1 S.card) _}))`.
    ...
    obtain ⟨hSne, hc_eq⟩ : S.Nonempty ∧ c = codedDistributionDataCode ((canonicalFinsetList S).map fun x =>
      { point := x, mass := ratMassInvNat (max 1 S.card) (by positivity) }) := by
          grind +locals;
    -- Since `S.Nonempty`, `max 1 S.card = S.card`, so by `codedUniformOn_code_eq` the right side equals `(codedUniformOn S hSne).code`; hence `c = (codedUniformOn S hSne).code`.
    have hc_eq' : c = (codedUniformOn S hSne).code := by
      convert hc_eq using 1;
      convert codedUniformOn_code_eq S hSne using 1;
      grind +extAll;
    -- Now `codedUniformOn S hSne` is a probability (`codedUniformOn_isProbability`), so `probModelOfCode c = probModelOfCode (codedUniformOn S hSne).code = codedUniformOn S hSne` by `probModelOfCode_eq`.
  ...
  · obtain ⟨hSne, hc⟩ := h;
    have hS : ((decodeDistributionData c).map CodedDistributionEntry.point).toFinset = (probModelOfCode c).support := by
      rw [ hc, codedUniformOn_code_eq ];
      grind +suggestions;
    unfold isCanonicalUniformCodeBool; simp +decide [ hSne, hS ] ;
    convert hc using 1
...
theorem isCanonicalUniformCodeBool_primrec : Primrec isCanonicalUniformCodeBool := by
  ...
  · ext; simp [isCanonicalUniformCodeBool]
...
theorem snapshotRichElementsList_toFinset (c : Code) (i j k t : ℕ) :
  ...
  · obtain ⟨ ⟨ S, hS₁, hS₂ ⟩, hS₃ ⟩ := h;
    ...
    have h_card_le_length : ∀ {l : List (List BitString)}, List.Nodup (List.map List.toFinset l) → ∀ y, (Finset.filter (fun S => y ∈ S) (List.toFinset (List.map List.toFinset l))).card ≤ (List.filter (fun S => decide (y ∈ S)) l).length := by
      ...
      split_ifs <;> simp_all +decide [ Finset.filter_insert ];
    convert h_card_le_length _ y using 1;
    · congr! 1;
      ext; simp [snapshotDescList_nodup_map_toFinset];
...
theorem snapshotRichElementsList_primrec (c : Code) :
  ...
  · convert list_filter_primrec _ _ using 1;
    · have h1 : Primrec (fun p : (ℕ × ℕ) × ℕ =>
          ((canonicalFinsetList ((snapshotCodes c p.1.1 p.1.2).filter isCanonicalUniformCodeBool).toFinset).map
            (fun w => canonicalFinsetList (((decodeDistributionData w).map CodedDistributionEntry.point).toFinset))).filter
              (fun S => S.length ≤ 2 ^ p.2)) := by
                convert snapshotDescriptionsAndSizeLe_primrec c using 1;
      convert Primrec.list_flatten.comp ( h1.comp ( Primrec.pair ( Primrec.fst.comp ( Primrec.fst ) |> Primrec.pair <| Primrec.snd.comp ( Primrec.snd ) ) <| Primrec.snd.comp ( Primrec.fst ) ) ) using 1;
    · have h_countP : Primrec₂ (fun (p : (ℕ × ℕ) × ℕ × ℕ) (y : BitString) => (snapshotDescList c p.1.1 p.1.2 p.2.2).countP (fun S => decide (y ∈ S))) := by
      ...
      convert PrimrecPred.decide ( Primrec.nat_le.comp ( h_twoPow.comp Primrec.fst ) ( h_countP.comp ( Primrec.fst ) ( Primrec.snd ) ) ) using 1;
...
theorem snapshotDescriptions_eq_descriptionsWithComplexityLe {c : Code} {U : Map}
  ...
  · obtain ⟨c', hc', hc'_T⟩ : ∃ c' ∈ modelsWithComplexityLe U i, isCanonicalUniformCode c' ∧ (probModelOfCode c').support = T := by
      contrapose! hT; simp_all +decide [ descriptionsWithComplexityLe ] ;
      grind;
    ...
    have h_code_mem_snapshot : isCanonicalUniformCodeBool c' := by
      exact isCanonicalUniformCodeBool_iff c' |>.2 hc'_T.1;
    have h_code_mem_snapshot : ((decodeDistributionData c').map CodedDistributionEntry.point).toFinset = (probModelOfCode c').support := by
...
theorem partrec_richSelectorFn (c : Code) : Partrec (richSelectorFn c) := by
  ...
  · have h_check : Computable (fun p : ℕ × ℕ => decide (p.1 = p.2)) := by
    ...
    convert h_check.comp ( Computable.pair ( countHalts_computable c |> Computable.comp <| selNat_computable.comp Computable.fst |> Computable.pair <| Computable.snd ) ( selH_computable.comp Computable.fst ) ) using 1;
  · convert Primrec.to_comp _;
    convert codedUniformEncoder_primrec.comp _ using 1;
    convert snapshotRichElements_primrec c using 1;
    constructor <;> intro h;
    · convert snapshotRichElements_primrec c using 1;
    · convert h.comp ( Primrec.pair ( Primrec.pair ( selNat_primrec.comp Primrec.fst ) ( selAlpha_primrec.comp Primrec.fst ) ) ( Primrec.pair ( selMaxK_primrec.comp Primrec.fst ) ( Primrec.snd ) ) ) using 1
...
theorem richInput_KPPlain_le (U : Map) (hU : IsOptimalPrefixConditional U) (c_partrec : ℕ) :
  ...
  refine ⟨ 19 + c₀ + c_partrec, fun i j k h hh => le_trans ( add_le_add ( hc₀ _ ) le_rfl ) ?_ ⟩ ; norm_cast ; simp +decide [ richInput ];
...
theorem richInput_KPPlain_le_addr (U : Map) (hU : IsOptimalPrefixConditional U) (c_partrec : ℕ) :
  ...
  unfold logSlack; norm_cast; simp +arith +decide [ richInput ] ;
...
theorem emittedHalfRichChunks_fold_step_primrec :
  ...
  have h_step_primrec : Primrec (fun p : ℕ × (List BitString × (List (List BitString) × BitString)) => emittedHalfRichChunks_fold_step p.1 p.2.1 p.2.2.1 p.2.2.2) := by
    ...
    · convert Primrec.of_eq _ _;
      exact fun p => decide ( p.2.2.2 ∈ p.2.2.1.flatten );
      · have h_mem : Primrec₂ (fun (x : BitString) (L : List BitString) => decide (x ∈ L)) := by
          exact bitString_mem_primrec.comp ( Primrec.fst ) ( Primrec.snd );
        convert h_mem.comp ( Primrec.snd.comp ( Primrec.snd.comp ( Primrec.snd ) ) ) ( Primrec.list_flatten.comp ( Primrec.fst.comp ( Primrec.snd.comp ( Primrec.snd ) ) ) ) using 1;
      · grind;
    · exact Primrec.fst.comp ( Primrec.snd.comp ( Primrec.snd ) );
    · -- The function that appends the new chunk to the list of chunks is primitive recursive.
      have h_append_primrec : Primrec (fun p : List (List BitString) × (Nat × BitString × List BitString) => p.1 ++ [List.take (2 ^ p.2.1) (p.2.2.1 :: List.filter (fun y => bif decide (y = p.2.2.1) then false else true) (List.filter (fun y => bif decide (y ∈ p.1.flatten) then false else true) p.2.2.2))]) := by
        apply Primrec₂.comp;
        · exact Primrec.list_append.comp ( Primrec.fst ) ( Primrec.snd );
        · exact Primrec.fst;
        · convert Primrec.list_cons.comp _ _ using 1;
          · apply Primrec₂.comp;
            · grind +suggestions;
            · exact twoPow_primrec.comp ( Primrec.fst.comp ( Primrec.snd ) );
            · convert Primrec.list_cons.comp _ _ using 1;
              ...
              · convert list_filter_primrec _ _ using 1;
                · convert list_filter_primrec _ _ using 1;
                  ...
                  · convert Primrec.cond _ _ _ using 1;
                    · convert bitString_mem_primrec.comp _ _ using 1;
                      · exact Primrec.snd;
                      · exact Primrec.list_flatten.comp ( Primrec.fst.comp ( Primrec.fst ) );
                    · exact Primrec.const false;
                    · exact Primrec.const Bool.true;
                · convert Primrec.cond _ _ _ using 1;
                  · convert PrimrecPred.decide ( Primrec.eq.comp _ _ ) using 1;
                  ...
                  · exact Primrec.const Bool.true;
          · exact Primrec.const [];
      convert h_append_primrec.comp _ using 1;
      rotate_left;
      exact fun p => ( p.2.2.1, p.1, p.2.2.2, p.2.1 );
      · exact Primrec.pair ( Primrec.fst.comp ( Primrec.snd.comp ( Primrec.snd ) ) ) ( Primrec.pair ( Primrec.fst ) ( Primrec.pair ( Primrec.snd.comp ( Primrec.snd.comp ( Primrec.snd ) ) ) ( Primrec.fst.comp ( Primrec.snd ) ) ) );
  ...
  · exact Primrec.pair ( Primrec.fst.comp ( Primrec.fst ) ) ( Primrec.pair ( Primrec.fst.comp ( Primrec.snd.comp ( Primrec.fst ) ) ) ( Primrec.pair ( Primrec.snd.comp ( Primrec.snd.comp ( Primrec.fst ) ) ) ( Primrec.snd ) ) );
  · bound
...
def emittedHalfRichChunks_step (c : Code) (i j k : ℕ) (t : ℕ) (chunks : List (List BitString)) : List (List BitString) :=
...
theorem emittedHalfRichChunksList_primrec (c : Code) :
  ...
  apply Primrec.of_eq;
  rotate_right;
  exact fun p => ( List.foldl ( fun chunks t => emittedHalfRichChunks_step c p.1.1.1 p.1.1.2 p.1.2 ( t + 1 ) chunks ) ( emittedHalfRichChunks_step c p.1.1.1 p.1.1.2 p.1.2 0 [ ] ) ( List.range p.2 ) );
  · apply list_foldl_primrec;
...
theorem emittedHalfRichChunks_foldStep_flatten_mem (j : ℕ) (half_rich : List BitString)
    ...
    ∀ y ∈ (L.foldl (emittedHalfRichChunks_fold_step j half_rich) acc).flatten,
      y ∈ half_rich := by
  have h_ind : ∀ (xs : List BitString) (acc : List (List BitString)), (∀ y ∈ xs, y ∈ half_rich) → (∀ y ∈ acc.flatten, y ∈ half_rich) → (∀ y ∈ (xs.foldl (emittedHalfRichChunks_fold_step j half_rich) acc).flatten, y ∈ half_rich) := by
    intros xs acc hxs hacc y hy; induction xs using List.reverseRecOn generalizing acc
    case' nil => exact hacc y hy
    case' append_singleton xs x ih =>
      simp only [List.foldl_append, List.foldl_cons, List.foldl_nil, List.mem_flatten] at hy; obtain ⟨ l, hl₁, hl₂ ⟩ := hy; unfold emittedHalfRichChunks_fold_step at hl₁; by_cases hx : x ∈ ( List.foldl ( emittedHalfRichChunks_fold_step j half_rich ) acc xs ).flatten <;> simp_all +decide ;
      · convert ih acc hacc l _ hl₂ using 1;
        unfold emittedHalfRichChunks_fold_step at *; aesop;
      · unfold emittedHalfRichChunks_fold_step at *; by_cases hx : ∃ l ∈ List.foldl ( fun chunks x => bif decide ( ∃ l ∈ chunks, x ∈ l ) then chunks else chunks ++ [ List.take ( 2 ^ j ) ( x :: List.filter ( fun a => !decide ( a = x ) && !decide ( ∃ l ∈ chunks, a ∈ l ) ) half_rich ) ] ) acc xs, x ∈ l <;> simp_all +decide ;
        · grind;
...
theorem emittedHalfRichChunks_foldStep_flatten_nodup (j : ℕ) (half_rich : List BitString)
  ...
  case' append_singleton L ih ih_hyp =>
    unfold emittedHalfRichChunks_fold_step at *; by_cases hx : ih ∈ ( List.foldl ( emittedHalfRichChunks_fold_step j half_rich ) acc L ).flatten <;> simp_all +decide ;
    · unfold emittedHalfRichChunks_fold_step at *; aesop;
...
theorem emittedHalfRichChunksList_flatten_nodup (c : Code) (i j k t : ℕ) :
  ...
  induction t with
  | zero =>
    have h_eraseDups_nodup : ∀ (L : List BitString), List.Nodup (L.eraseDups) := by
      intro L; exact (by
      -- By definition of `List.eraseDupsBy.loop`, the result is a list with no duplicates.
      have h_eraseDupsBy_loop_nodup : ∀ (l : List BitString) (acc : List BitString), List.Nodup acc → List.Nodup (List.eraseDupsBy.loop (fun x1 x2 => x1 == x2) l acc) := by
        intros l acc hacc; induction l generalizing acc with
        | nil => simp_all +decide [ List.eraseDupsBy.loop ]
        | cons x l ih =>
          simp_all +decide [ List.eraseDupsBy.loop ]
          cases h : acc.any fun x2 => x == x2 <;> simp_all +decide
          grind
      exact h_eraseDupsBy_loop_nodup _ _ ( by simp +decide ));
    exact emittedHalfRichChunks_foldStep_flatten_nodup j _ ( h_eraseDups_nodup _ ) _ _ ( by simp +decide );
  | succ t ih =>
    convert emittedHalfRichChunks_foldStep_flatten_nodup j _ _ _ _ ih using 1;
    have h_eraseDups_nodup : ∀ {L : List BitString}, List.Nodup (List.eraseDups L) := by
      intro L; exact (by
      have h_eraseDups_nodup : ∀ (L : List BitString) (acc : List BitString), List.Nodup acc → List.Nodup (List.eraseDupsBy.loop (fun x1 x2 => x1 == x2) L acc) := by
        intros L acc hacc; induction L generalizing acc with
        | nil => simp_all +decide [ List.eraseDupsBy.loop ]
        | cons x L ih =>
          simp_all +decide [ List.eraseDupsBy.loop ]
          cases h : acc.any fun x2 => x == x2 <;> simp_all +decide
          grind
      exact h_eraseDups_nodup _ _ ( by simp +decide ));
    exact h_eraseDups_nodup
...
theorem runOut_mono {c : Code} {t t' : ℕ} (h : t ≤ t') {p w : BitString}
  ...
  convert Option.bind_eq_some_iff.mp hw using 1;
  constructor <;> intro h <;> simp_all +decide [ runOut ];
  · cases h : Code.evaln t c ( Nat.pair ( Encodable.encode p ) 0 ) <;> aesop;
  · obtain ⟨ a, ha₁, ha₂ ⟩ := h; rw [ show Code.evaln t' c ( Nat.pair ( Encodable.encode p ) 0 ) = some a from ?_ ] ; aesop;
    convert Nat.Partrec.Code.evaln_mono h _;
    exact ha₁.symm ▸ by simp +decide ;
...
theorem snapshotDescriptionsAndSizeLe_subset_of_le (c : Code) (i j : ℕ) {t t' : ℕ} (h : t ≤ t') :
  ...
  unfold snapshotDescriptionsAndSizeLe; simp_all +decide [ Finset.subset_iff ];
...
theorem emittedHalfRichChunks_foldStep_flatten_mem_or (j : ℕ) (half_rich : List BitString)
  ...
  | append_singleton L ih =>
    unfold emittedHalfRichChunks_fold_step; simp +decide [ * ] ;
...
theorem snapshotRichElementsList_subset_threshold (c : Code) (i j t : ℕ) {m m' : ℕ}
  ...
  unfold snapshotRichElementsList at *; simp_all +decide [ List.mem_filter ] ;
...
theorem emittedHalfRichChunksList_flatten_half_rich (c : Code) (i j k t : ℕ) :
  ...
  | zero =>
    intro y hy
    simp [emittedHalfRichChunksList, emittedHalfRichChunks_step] at hy;
  ...
  | succ t ih =>
    ...
    · rename_i h;
      convert mem_snapshotRichElementsList_multiplicity c i j ( k - 1 ) ( t + 1 ) _ using 1;
      have h_eraseDups : ∀ {L : List BitString}, y ∈ L.eraseDups → y ∈ L := by
        intros L hL; induction L using List.reverseRecOn
        case' nil => simp_all +decide
        case' append_singleton L ih _ => simp_all +decide [ List.eraseDups_append ]
        simp_all +decide [ List.removeAll ];
...
theorem emittedHalfRichChunksList_mem_ne_nil (c : Code) (i j k t : ℕ) :
  ...
  | zero =>
    ...
    have h_foldl_nonempty : ∀ (M : List BitString) (acc : List (List BitString)), (∀ l ∈ acc, l ≠ []) → (∀ l ∈ List.foldl (emittedHalfRichChunks_fold_step j (snapshotRichElementsList c i j (k - 1) 0).eraseDups) acc M, l ≠ []) := by
      intros M acc hacc l hl; induction M using List.reverseRecOn generalizing acc
      case' nil => simp_all +decide
      case' append_singleton M ih _ => simp_all +decide [ emittedHalfRichChunks_fold_step ]
      by_cases h : ∃ l ∈ List.foldl ( emittedHalfRichChunks_fold_step j ( snapshotRichElementsList c i j ( k - 1 ) 0 ).eraseDups ) acc M, ih ∈ l <;> simp_all +decide;
      · grind;
      · rcases n : 2 ^ j with ( _ | _ | n ) <;> simp_all +decide [ List.take ]; all_goals grind;
    exact fun h => h_foldl_nonempty _ _ ( by simp +decide ) _ h rfl;
  | succ t ih =>
    have h_fold : ∀ (L : List BitString) (acc : List (List BitString)), (∀ l ∈ acc, l ≠ []) → (∀ l ∈ L.foldl (emittedHalfRichChunks_fold_step j ((snapshotRichElementsList c i j (k - 1) (t + 1)).eraseDups)) acc, l ≠ []) := by
      intros L acc hacc l hl; induction L using List.reverseRecOn generalizing acc
      case' nil => simp_all +decide
      case' append_singleton L ih _ => simp_all +decide [ emittedHalfRichChunks_fold_step ]
      -- By definition of `emittedHalfRichChunks_fold_step`, if `l` is in the fold step result, then `l` is either in the fold step result of `L` or in the new chunk.
      by_cases h : ∃ l ∈ List.foldl (emittedHalfRichChunks_fold_step j (snapshotRichElementsList c i j (k - 1) (t + 1)).eraseDups) acc L, ‹BitString› ∈ l;
      · grind;
      · aesop;
    exact h_fold _ _ ih
...
theorem emittedHalfRichChunks_nodup (c : Code) (i j k t : ℕ) :
  ...
  · rw [ List.pairwise_map ];
    ...
    intro a b ha hb hab h; have := List.disjoint_iff_ne.mp hab; simp_all +decide [ Finset.ext_iff ] ;
    ...
    have hcore : ∑ S ∈ Full.toFinset, ∑ y ∈ S, (desc.countP (fun S' => decide (y ∈ S'))) ≤ D * 2 ^ j := by
      have hcore_upper : ∀ S' ∈ desc.toFinset, ∑ S ∈ Full.toFinset, (S ∩ S'.toFinset).card ≤ S'.length := by
        ...
        have h_disjoint : ∀ S₁ S₂, S₁ ∈ Full.toFinset → S₂ ∈ Full.toFinset → S₁ ≠ S₂ → Disjoint S₁ S₂ := by
          ...
          have h_disjoint : ∀ l₁ l₂, l₁ ∈ emittedHalfRichChunksList c i j k t → l₂ ∈ emittedHalfRichChunksList c i j k t → l₁ ≠ l₂ → Disjoint l₁.toFinset l₂.toFinset := by
            ...
            · grind;
          simp +zetaDelta at *;
          obtain ⟨ l₁, hl₁, rfl ⟩ := List.mem_map.mp hS₁.1; obtain ⟨ l₂, hl₂, rfl ⟩ := List.mem_map.mp hS₂.1; specialize h_disjoint l₁ l₂ hl₁ hl₂; aesop;
      ...
      have hcore_upper : ∑ S ∈ Full.toFinset, ∑ y ∈ S, (desc.countP (fun S' => decide (y ∈ S'))) = ∑ S' ∈ desc.toFinset, ∑ S ∈ Full.toFinset, (S ∩ S'.toFinset).card := by
        ...
        have hcore_upper : ∀ y ∈ S, (desc.countP (fun S' => decide (y ∈ S'))) = ∑ S' ∈ desc.toFinset, (if y ∈ S' then 1 else 0) := by
          intros y hy; rw [ List.countP_eq_length_filter ] ; simp +decide ;
      ...
      · simp +zetaDelta at *;
        unfold snapshotDescList at hx; simp_all +decide [ List.mem_filter ] ;
      · simp +zetaDelta at *;
        exact List.toFinset_card_le _;
    nlinarith [ pow_pos ( zero_lt_two' ℕ ) j ];
  cases k <;> simp_all +decide [ pow_add ];
...
theorem emittedHalfRichChunks_foldStep_unplaced_full (j : ℕ) (half_rich L : List BitString)
  ...
  case' nil => simp_all +decide
  case' append_singleton L ih _ => simp_all +decide [ emittedHalfRichChunks_fold_step ]
  rename_i h; specialize h acc y hy_hr; by_cases h' : ∃ l ∈ List.foldl ( emittedHalfRichChunks_fold_step j half_rich ) acc L, ih ∈ l <;> simp_all +decide ;
  by_contra h_contra;
  have h_new_chunk_length : y ∈ List.take (2 ^ j) (ih :: List.filter (fun a => !decide (a = ih) && !decide (∃ l ∈ List.foldl (emittedHalfRichChunks_fold_step j half_rich) acc L, a ∈ l)) half_rich) := by
...
theorem nonfull_chunk_emit_time (c : Code) (i j k : ℕ) (t : ℕ) (S : Finset BitString)
  ...
  have hx_unplaced_fold : x ∉ (List.foldl (emittedHalfRichChunks_fold_step j ((snapshotRichElementsList c i j (k - 1) t).eraseDups)) (if t > 0 then emittedHalfRichChunksList c i j k (t - 1) else []) (snapshotRichElementsList c i j k t).eraseDups).flatten := by
    cases t <;> simp_all +decide [ emittedHalfRichChunksList ]; all_goals unfold emittedHalfRichChunks_step at *; aesop;
...
theorem trigger_gains_fresh_finset (c : Code) (i j k : ℕ) (t₁ t₂ : ℕ) (y : BitString)
  ...
  have h_count_ge : ((snapshotDescriptionsAndSizeLe c i j t₂).filter (fun S => y ∈ S)).card ≥ 2 ^ k := by
    have := mem_snapshotRichElementsList_multiplicity c i j k t₂ h_rich; simp_all +decide [ snapshotDescList_countP_eq_card ] ;
  rcases k with ( _ | k ) <;> simp_all +decide [ pow_succ' ];
  · rw [ Finset.card_eq_zero.mpr ] <;> aesop;
  · linarith [ show Finset.card ( Finset.filter ( fun S => y ∈ S ) ( snapshotDescriptionsAndSizeLe c i j t₁ ) ∩ Finset.filter ( fun S => y ∈ S ) ( snapshotDescriptionsAndSizeLe c i j t₂ ) ) ≤ Finset.card ( Finset.filter ( fun S => y ∈ S ) ( snapshotDescriptionsAndSizeLe c i j t₁ ) ) from Finset.card_le_card fun x hx => by aesop ]
...
theorem emittedHalfRichChunks_foldStep_nonfull_count (j : ℕ) (half_rich : List BitString)
  ...
  case' nil => simp_all +decide
  case' append_singleton L ih _ => simp_all +decide [ emittedHalfRichChunks_fold_step ]
  by_cases h : ∃ l ∈ List.foldl ( emittedHalfRichChunks_fold_step j half_rich ) acc L, ih ∈ l <;> simp_all +decide;
  rename_i h';
  specialize h' acc;
  by_cases h'' : ( List.filter ( fun l => decide ( l.length < 2 ^ j ) ) acc ).length < ( List.filter ( fun l => decide ( l.length < 2 ^ j ) ) ( List.foldl ( emittedHalfRichChunks_fold_step j half_rich ) acc L ) ).length <;> simp_all +decide;
  by_cases h''' : 2 ^ j ≤ List.length ( ih :: List.filter ( fun a => !decide ( a = ih ) && !decide ( ∃ l ∈ List.foldl ( emittedHalfRichChunks_fold_step j half_rich ) acc L, a ∈ l ) ) half_rich ) <;> simp_all +decide;
  · grind;
  · rw [ decide_eq_false ] <;> simp_all +decide;
    intro h y hy; exact (by
    by_cases hy' : y ∈ List.flatten ( List.foldl ( emittedHalfRichChunks_fold_step j half_rich ) acc L ) <;> simp_all +decide [ List.mem_flatten ];
    · exact ⟨ _, Or.inl hy'.choose_spec.1, hy'.choose_spec.2 ⟩;
    · use List.take (2 ^ j) (ih :: List.filter (fun a => !decide (a = ih) && !decide (∃ l ∈ List.foldl (emittedHalfRichChunks_fold_step j half_rich) acc L, a ∈ l)) half_rich);
...
theorem emittedHalfRichChunksList_numNonfull_succ_le (c : Code) (i j k t : ℕ) :
  ...
  convert emittedHalfRichChunks_foldStep_nonfull_count j _ _ _ _ |> And.left using 1;
  have h_eraseDups : ∀ {L : List BitString} {x : BitString}, x ∈ L.eraseDups ↔ x ∈ L := by
    ...
    simp_all +decide [ List.removeAll ];
    grind;
  exact fun x hx => h_eraseDups.mpr ( snapshotRichElementsList_subset_threshold _ _ _ _ ( Nat.sub_le _ _ ) ( h_eraseDups.mp hx ) )
...
theorem emittedHalfRichChunksList_numNonfull_zero_le (c : Code) (i j k : ℕ) :
  ...
  unfold emittedHalfRichChunksList; simp +decide [ emittedHalfRichChunks_step ] ;
  convert emittedHalfRichChunks_foldStep_nonfull_count j _ _ _ _ |> And.left using 1 ; simp +decide [ snapshotRichElementsList_zero ]
...
theorem nonfullChunkEmissionTime_strict_mono (c : Code) (i j k t m : ℕ)
  ...
  obtain ⟨l₁, hl₁⟩ : ∃ l₁, (List.drop (m + 1) (List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k t))).head? = some l₁ := by
    simp_all +decide [ emittedHalfRich_nonfull_len_eq ];
  have h_mem_iff_count : ∀ τ ≤ t, l_m ∈ emittedHalfRichChunksList c i j k τ ↔ m < ((List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k τ)).length) := by
    intros τ hτ
    have h_mem_iff_count : l_m ∈ List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k τ) ↔ m < ((List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k τ)).length) := by
      have h_mem_iff_count : List.Nodup (List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k t)) := by
        exact List.Nodup.filter _ ( emittedHalfRichChunksList_Nodup c i j k t );
      obtain ⟨rest, hrest⟩ := emittedHalfRichChunksList_filter_prefix c i j k hτ (fun l => l.length < 2 ^ j);
      rw [ List.head?_drop ] at hl_m;
      grind;
    simp +zetaDelta at *;
    grind;
  have h_mem_iff_count₁ : ∀ τ ≤ t, l₁ ∈ emittedHalfRichChunksList c i j k τ ↔ m + 1 < ((List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k τ)).length) := by
    intro τ hτ;
    have h_mem_iff_count₁ : l₁ ∈ List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k τ) ↔ m + 1 < ((List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k τ)).length) := by
      have h_mem_iff_count₁ : List.Nodup (List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k t)) := by
        exact List.Nodup.filter _ ( emittedHalfRichChunksList_Nodup c i j k t );
      obtain ⟨rest, hrest⟩ := emittedHalfRichChunksList_filter_prefix c i j k hτ (fun l => l.length < 2 ^ j);
...
theorem nonfullChunkTrigger_rich_curr (c : Code) (i j k t m : ℕ)
  ...
  obtain ⟨l, hl⟩ : ∃ l, l ∈ List.drop m (List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k t)) ∧ l ≠ [] ∧ (List.drop m (List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k t))).head? = some l := by
    obtain ⟨l, hl⟩ : ∃ l, l ∈ List.drop m (List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k t)) ∧ (List.drop m (List.filter (fun l => l.length < 2 ^ j) (emittedHalfRichChunksList c i j k t))).head? = some l := by
      unfold emittedHalfRichChunks at h_lt; simp_all +decide ;
    ...
    refine ⟨ l, hl.1, ?_, hl.2 ⟩ ; intro h ; simp_all +decide ;
    have := emittedHalfRichChunksList_mem_ne_nil c i j k t; simp_all +decide ;
    exact this _ ( List.mem_of_mem_drop hl.1 |> List.mem_of_mem_filter ) rfl;
  ...
  have h_l_head : ∃ x ∈ (snapshotRichElementsList c i j k τ).eraseDups, l ≠ [] ∧ l.head? = some x := by
    have h_l_head : ∀ {L : List BitString} {acc : List (List BitString)}, l ∈ List.foldl (emittedHalfRichChunks_fold_step j ((snapshotRichElementsList c i j (k - 1) τ).eraseDups)) acc L → l ∉ acc → ∃ x ∈ L, l ≠ [] ∧ l.head? = some x := by
      intros L acc hl hacc; induction L using List.reverseRecOn generalizing acc
      case' nil => simp_all +decide
      case' append_singleton L ih _ => simp_all +decide [ emittedHalfRichChunks_fold_step ]
      by_cases h : ∃ l' ∈ List.foldl (emittedHalfRichChunks_fold_step j (snapshotRichElementsList c i j (k - 1) τ).eraseDups) acc L, ih ∈ l' <;> simp_all +decide;
      · grind;
      · rcases n : 2 ^ j with ( _ | _ | n ) <;> simp_all +decide [ List.take ]; all_goals grind;
    rcases τ with ( _ | τ ) <;> simp_all +decide [ emittedHalfRichChunksList ];
    · unfold emittedHalfRichChunks_step at hτ; aesop;
    · exact h_l_head hτ.2.1 ( hτ.2.2 _ le_rfl );
  ...
  unfold emittedHalfRichChunks at *; simp_all +decide ;
  rw [ show ( List.find? ( fun τ => decide ( l ∈ emittedHalfRichChunksList c i j k τ ) ) ( List.range ( t + 1 ) ) ) = some τ from ?_ ];
  · cases l <;> aesop;
  · rw [ List.find?_eq_some_iff_append.mpr ];
    refine ⟨ by aesop, List.range τ, List.drop ( τ + 1 ) ( List.range ( t + 1 ) ), ?_, ?_ ⟩ <;> simp_all +decide [ List.range_succ ];
    rw [ ← List.take_append_drop ( τ + 1 ) ( List.range t ++ [ t ] ) ] ; simp +decide [ List.take_append, List.drop_append ] ;
    cases min_cases ( τ + 1 ) t <;> simp_all +decide [ List.range_succ ];
    cases ‹t ≤ τ + 1 ∧ t ≤ τ›.2.eq_or_lt <;> first | linarith | aesop;
...
theorem onlineHalfRichChunkBody_primrec (c : Code) :
  ...
  have h_list_drop_primrec : Primrec (fun p : BitString × ℕ => List.drop (selH p.1) (emittedHalfRichChunksList c (selNat p.1) (selAlpha p.1) (selMaxK p.1) p.2)) := by
    ...
    have h_drop_primrec : Primrec (fun p : ℕ × ℕ × ℕ × ℕ => emittedHalfRichChunksList c p.1 p.2.1 p.2.2.1 p.2.2.2) := by
      ...
      rotate_left;
      exact fun p => ( ( ( p.1, p.2.1 ), p.2.2.1 ), p.2.2.2 );
    ...
    · grind;
  convert Primrec.list_casesOn h_list_drop_primrec _ _ using 1;
  rotate_left;
  exact fun _ => [];
  exact fun p q => codedDistributionDataCode ( List.map ( fun x => { point := x, mass := ratMassInvNat ( max 1 ( canonicalFinsetList q.1.toFinset ).length ) ( by positivity ) } ) ( canonicalFinsetList q.1.toFinset ) );
  · exact Primrec.const [];
  · convert isCanonicalUniformCodeBool_primrec using 1;
    constructor <;> intro h;
    · convert isCanonicalUniformCodeBool_primrec using 1;
    · convert Primrec.of_eq _ _;
      exact fun p => codedDistributionDataCode ( List.map ( fun x => { point := x, mass := ratMassInvNat ( max 1 ( canonicalFinsetList p.2.1.toFinset ).length ) ( by positivity ) } ) ( canonicalFinsetList p.2.1.toFinset ) );
      · convert codedUniformEncoder_primrec.comp _ using 1;
        exact canonicalFinsetList_toFinset_primrec.comp ( Primrec.fst.comp ( Primrec.snd ) );
      · grind;
...
theorem partrec_onlineHalfRichChunkSelectorFn (c : Code) : Partrec (onlineHalfRichChunkSelectorFn c) := by
  ...
  · convert Partrec.rfind _;
    convert Primrec.to_comp _;
    convert PrimrecPred.decide ( Primrec.nat_lt.comp ( selH_primrec.comp ( Primrec.fst ) ) ( Primrec.list_length.comp ( emittedHalfRichChunksList_primrec c |> Primrec.comp <| _ ) ) ) using 1;
    rotate_left;
    exact fun p => ( ( ( selNat p.1, selAlpha p.1 ), selMaxK p.1 ), p.2 );
    · exact Primrec.pair ( Primrec.pair ( Primrec.pair ( selNat_primrec.comp ( Primrec.fst ) ) ( selAlpha_primrec.comp ( Primrec.fst ) ) ) ( selMaxK_primrec.comp ( Primrec.fst ) ) ) ( Primrec.snd );
    · grind +extAll;
  · convert onlineHalfRichChunkBody_primrec c |> Primrec.to_comp |> Partrec.comp <| ?_ using 1;
    exact Computable.id
...
theorem onlineHalfRichChunkSelectorFn_eq (c : Code) (i j k h t : ℕ)
  ...
  · rw [ List.drop_eq_getElem_cons ];
    convert codedUniformOn_code_eq _ _ using 1;
    grind;
    exact h_lt
...
theorem onlineHalfRichChunkSelectorFn_eq_of_mem (c : Code) (i j k h t : ℕ)
  ...
  apply Eq.symm; exact (by
    have := onlineHalfRichChunkSelectorFn_eq c i j k h (Nat.find (⟨t, h_lt⟩ : ∃ t, h < (emittedHalfRichChunksList c i j k t).length)) (Nat.find_spec (⟨t, h_lt⟩ : ∃ t, h < (emittedHalfRichChunksList c i j k t).length)) (fun t' ht' => Nat.find_min (⟨t, h_lt⟩ : ∃ t, h < (emittedHalfRichChunksList c i j k t).length) ht') (by
    grind +suggestions)
    grind +suggestions
  )
...
theorem halfRichComplexityPortion_selector_correct (U : Map) (hU : IsOptimalPrefixConditional U) :
  ...
  · convert onlineHalfRichChunkSelectorFn_eq_of_mem c i j k h' t hh'₁ _ using 1;
    grind;
    grind
