...
theorem snapshotRichElementsList_primrec (c : Code) :
  ...
  · convert list_filter_primrec _ _ using 1
    · have h1 : Primrec (fun p : (ℕ × ℕ) × ℕ =>
        ...
        (snapshotDescriptionsAndSizeLe_primrec c).of_eq (fun _ => rfl)
      convert Primrec.list_flatten.comp (h1.comp (Primrec.pair
        (Primrec.fst.comp Primrec.fst |> Primrec.pair <|
          Primrec.snd.comp Primrec.snd) <| Primrec.snd.comp Primrec.fst)) using 1
    · have h_countP : Primrec₂ (fun (p : (ℕ × ℕ) × ℕ × ℕ) (y : BitString) =>
      ...
      convert PrimrecPred.decide (Primrec.nat_le.comp (h_twoPow.comp Primrec.fst)
        (h_countP.comp Primrec.fst Primrec.snd)) using 1
...
theorem emittedHalfRichChunks_fold_step_primrec :
  ...
  have h_step_primrec : Primrec
    ...
    · convert Primrec.of_eq _ _;
      ...
      · have h_mem :
            Primrec₂ (fun (x : BitString) (L : List BitString) => decide (x ∈ L)) := by
          exact bitString_mem_primrec.comp ( Primrec.fst ) ( Primrec.snd );
        convert h_mem.comp (Primrec.snd.comp (Primrec.snd.comp Primrec.snd))
          (Primrec.list_flatten.comp (Primrec.fst.comp (Primrec.snd.comp Primrec.snd))) using 1
      · exact fun _ => rfl;
    ...
    · -- The function that appends the new chunk to the list of chunks is primitive recursive.
      have h_append_primrec : Primrec
        ...
        · convert Primrec.list_cons.comp _ _ using 1;
          · apply Primrec₂.comp;
            · exact Primrec.list_take.comp Primrec.snd Primrec.fst;
            · exact twoPow_primrec.comp ( Primrec.fst.comp ( Primrec.snd ) );
            · convert Primrec.list_cons.comp _ _ using 1;
              · exact Primrec.fst.comp ( Primrec.snd.comp ( Primrec.snd ) );
              · convert list_filter_primrec _ _ using 1;
...
theorem emittedHalfRichChunksList_numNonfull_succ_le (c : Code) (i j k t : ℕ) :
  ...
  have h_eraseDups : ∀ {L : List BitString} {x : BitString}, x ∈ L.eraseDups ↔ x ∈ L := by
    ...
    grind;
  exact fun x hx => h_eraseDups.mpr
    (snapshotRichElementsList_subset_threshold _ _ _ _ (Nat.sub_le _ _) (h_eraseDups.mp hx))
