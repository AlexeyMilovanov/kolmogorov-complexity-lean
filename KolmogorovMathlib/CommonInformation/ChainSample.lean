...
theorem chain_pair_projection_plainK_upper
  ...
  have hrecode : plainK V (pairCode (chainWordAt W i) (chainWordAt W j)) ≤
    ...
    rw [hcode]
    have h_eq : pairCode (chainWordAt W i) (chainWordAt W j) =
      pairCode (w.map Prod.fst) (w.map Prod.snd) := by
      dsimp [chainWordAt, w]
...
theorem chain_triple_projection_plainK_upper
  ...
  have hrecode : plainK V (pairCode (chainPairAt W i j) (chainWordAt W l)) ≤
    ...
    rw [hcode]
    have h_eq : pairCode (chainPairAt W i j) (chainWordAt W l) =
      pairCode (pairCode (w.map (fun p => p.1.1)) (w.map (fun p => p.1.2))) (w.map Prod.snd) := by
      dsimp [chainPairAt, chainWordAt, pairCode, w]
      rw [List.map_map, List.map_map, List.map_map]
      rfl
    rw [h_eq]
    exact hRecode w
