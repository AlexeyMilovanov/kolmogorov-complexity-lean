...
theorem commonInformationRegion_coords_pos {V : Map} {x y : BitString}
  ...
  refine ⟨?_, ?_, ?_⟩
  · have h : (0 : ENat) < (t.1 : ENat) := lt_of_le_of_lt (zero_le _) hz
    exact_mod_cast h
  · have h : (0 : ENat) < (t.2.1 : ENat) := lt_of_le_of_lt (zero_le _) hx
    exact_mod_cast h
  · have h : (0 : ENat) < (t.2.2 : ENat) := lt_of_le_of_lt (zero_le _) hy
    exact_mod_cast h
