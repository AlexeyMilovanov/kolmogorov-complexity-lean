...
theorem familyCandidateCodes_computable (c : Code) (𝒜 : PreDescriptionFamily) :
  ...
  convert h_filter.comp ( Computable.pair ( ( candidateCodes_primrec c ).to_comp ) ( 𝒜.enumeration.computable.comp ( Computable.snd ) ) ) using 1
...
theorem familyAppearanceListCodes_computable (c : Code) (𝒜 : PreDescriptionFamily) :
  ...
  rotate_left;
  exact fun p => p.2;
  exact fun p => ( familyCandidateCodes c p.1.1.1 𝒜 p.1.1.2 p.1.2 0 ).eraseDups;
  exact fun p q => ( q.2 ++ familyCandidateCodes c p.1.1.1 𝒜 p.1.1.2 p.1.2 ( q.1 + 1 ) ).eraseDups;
  · exact Computable.snd;
  · have h_eraseDups : Primrec (fun l : List BitString => l.eraseDups) :=
      eraseDups_bitstring_primrec
    convert Computable.comp ( h_eraseDups.to_comp ) ( familyCandidateCodes_computable c 𝒜 |> Computable.comp <| Computable.pair ( Computable.fst ) ( Computable.const 0 ) ) using 1;
  · have h_eraseDups : Computable (fun l : List BitString => l.eraseDups) :=
      eraseDups_bitstring_primrec.to_comp
    convert h_eraseDups.comp ( Computable.list_append.comp ( Computable.snd.comp ( Computable.snd ) ) ( familyCandidateCodes_computable c 𝒜 |> Computable.comp <| Computable.pair ( Computable.fst.comp <| Computable.fst ) <| Computable.succ.comp <| Computable.fst.comp <| Computable.snd ) ) using 1;
  · funext p
    obtain ⟨p₁, t⟩ := p
    induction t with
...
theorem exists_familyCandidate_of_mem_familyAppearanceListCodes
  ...
  | succ t ih =>
    ...
    rw [ mem_eraseDups_bitString ] ; simp +decide [ hw ];
