...
Release validation discovers six maximal module roots:
`KolmogorovMathlib`,
...
`KolmogorovMathlib.Restricted.FamilyCurve.GoodSets`. This keeps standalone
modules in the build even when they are not reachable from the aggregate root.

This branch is pinned to Lean `v4.28.0` and the matching Mathlib ecosystem.

## Branches

This `lean-4.28-aristotle` branch preserves the Lean `v4.28.0` version. The
repository's `main` branch contains the current Lean `v4.32.1` version.
