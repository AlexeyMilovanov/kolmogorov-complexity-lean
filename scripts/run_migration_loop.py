...
def collect_context(section: dict[str, Any], root: Path) -> str:
    ...
    if section.get("workflow") == "migration":
        source_files = {
...
def is_migration(section: dict[str, Any]) -> bool:
    return section.get("workflow") == "migration"


def migration_reference_args() -> list[str]:
    if MIGRATION_SOURCE_ROOT.exists():
        return ["--add-dir", str(MIGRATION_SOURCE_ROOT)]
    return []
...
# Latest strategic migration plan
...
STATUS: MIGRATED / PARTIAL_MIGRATION / CLEANUP_IMPROVED / BUILD_BROKEN / INTERFACE_PROBLEM

## Changes Made
List exact files and declarations.

## Verification
List targeted and full builds, errors, warnings, and static scans.

## Remaining Migration
Name the next dependency-ready files and exact blockers.

## Risks
Report possible API drift, source results not yet represented, or reverted attempts.
...
def ordinary_prompt(
    ...
    if is_migration(section):
        return migration_ordinary_prompt(stage, section, iteration, context, previous, work_root)
...
def strategy_prompt(
    ...
    if is_migration(section):
        return migration_strategy_prompt(stage, section, iteration, context, previous, span, work_root)
    if is_polishing(section):
